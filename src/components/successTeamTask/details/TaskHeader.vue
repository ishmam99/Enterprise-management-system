...
const updateStatus = async (newStatus) => {
  try {
    await api().post(`/success-team-tasks/${props.task.id}`, {
      status: newStatus,
       _method:'put',
...
const showNotification = (message, type) => {
  ...
  notification.className = `fixed top-4 right-4 px-6 py-3 rounded-lg shadow-lg z-50 transform transition-all duration-300 ${
    type === 'success' ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-blue-50 text-blue-800 border border-blue-200'
  }`
  ...
  <div class="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
    ...
    <div class="flex flex-col sm:flex-row gap-3">
      ...
      <div class="relative group">
        <button
          class="flex items-center gap-2 px-4 py-2.5 rounded-lg font-medium transition-all duration-200 shadow-sm"
          :class="[
        ...
        <div class="absolute right-0 mt-2 w-48 rounded-xl bg-white shadow-lg border border-gray-100 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
          <div v-for="option in statusOptions" :key="option.value">
            <button
              @click="updateStatus(option.value)"
              class="w-full px-4 py-2.5 text-left hover:bg-gray-50 transition-colors flex items-center justify-between"
              :class="{
                'font-medium': task.status === option.value,
                'text-gray-700': task.status !== option.value
              }"
            >
              {{ option.label }}
              <i v-if="task.status === option.value" class="fas fa-check text-blue-500"></i>
            </button>
          </div>
        </div>
      </div>
      ...
      <!-- Actions Dropdown -->
      <div class="relative group">
        <button class="px-4 py-2.5 rounded-lg bg-white border border-gray-200 hover:bg-gray-50 transition-colors font-medium text-gray-700 shadow-sm hover:shadow-md flex items-center gap-2">
        ...
        <div class="absolute right-0 mt-2 w-48 rounded-xl bg-white shadow-lg border border-gray-100 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
          ...
          <button
            @click="router.push('/customer_success_manager/team-tasks')"
            class="w-full px-4 py-2.5 text-left hover:bg-gray-50 transition-colors flex items-center gap-2 text-gray-700"
          >
