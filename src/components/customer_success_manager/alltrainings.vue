      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gradient-to-r from-purple-600 to-pink-600 text-white">
            <tr>
              <th class="py-3 px-4 text-left text-xs font-semibold uppercase">#</th>
              <th class="py-3 px-4 text-left text-xs font-semibold uppercase">Training Course Name</th>
              <th class="py-3 px-4 text-left text-xs font-semibold uppercase">Trainer Name</th>
              <th class="py-3 px-4 text-left text-xs font-semibold uppercase">Industry</th>
              <th class="py-3 px-4 text-left text-xs font-semibold uppercase">Solution</th>
              <th class="py-3 px-4 text-left text-xs font-semibold uppercase">Software</th>
              <th class="py-3 px-4 text-center text-xs font-semibold uppercase">Duration</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            ...
            >
              <td class="py-3 px-4 font-medium text-sm">{{ index + 1 }}</td>
              <td class="py-3 px-4">
                <div class="font-semibold text-gray-800 text-sm">{{ training.event?.training_course.title }}</div>
              </td>
              <td class="py-3 px-4 text-gray-600 text-sm truncate max-w-[150px]" :title="training.event?.trainer?.user?.name">
                {{ training.event?.trainer?.user?.name }}
              </td>
              <td class="py-3 px-4 text-gray-600 text-sm truncate max-w-[120px]" :title="training.event?.training_course?.industry?.name">
                {{ training.event?.training_course?.industry?.name }}
              </td>
              <td class="py-3 px-4 text-sm truncate max-w-[120px]" :title="training.event?.training_course?.solution?.name">
                {{ training.event?.training_course?.solution?.name }}
              </td>
              <td class="py-3 px-4 text-sm truncate max-w-[120px]" :title="training.event?.training_course?.software?.name">
