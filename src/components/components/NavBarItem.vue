...
<template>
  ...
  <component
    ...
    v-else
    ref="root"
    class="block lg:flex items-center bg-blue-500 border-0 relative cursor-pointer"
  ...
  >
    ...
    <div class="flex items-center">
      <UserAvatarCurrentUser v-if="item?.isCurrentUser" class="w-10 h-10 mr-3 inline-flex" />
      <BaseIcon v-if="item.icon" :path="item.icon" class="transition-colors" />
      <p
        class="px-2 transition-colors text-white font-semibold"
    ...
    >
      <NavBarMenuList :menu="item.menu" @menu-click="menuClickDropdown" class="bg-white"/>
    </div>
  </component>
</template>
