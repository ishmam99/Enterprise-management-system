...
const getApiVariable = (value) => {
  if (value === 'deal-structure-analysis-service') {
    return 'A. Structure Analysis Service'
  } else if (value === 'deal-system-dynamics-analysis-service') {
    return 'B. System Dynamics Analysis Service'
  } else if (value === 'deal-acoustics-analysis-service') {
    return 'C. Acoustics Analysis Service'
  } else if (value === 'deal-fluids-analysis-service') {
    return 'D. Fluids Analysis Service'
  } else if (value === 'deal-autonomous-analysis-service') {
    return 'E. Autonomous Analysis Service'
  } else if (value === 'deal-vmc-analysis-service') {
    return 'F. VM&C Analysis Service'
  } else if (value === 'deal-icme-analysis-service') {
...
const getHeadingText = (value) => {
  if (value === 'deal-structure-analysis-service') {
    return 'Structure Analysis Service'
  } else if (value === 'deal-system-dynamics-analysis-service') {
    return 'System Dynamics Analysis Service'
  } else if (value === 'deal-acoustics-analysis-service') {
    return 'Acoustics Analysis Service'
  } else if (value === 'deal-fluids-analysis-service') {
    return 'Fluids Analysis Service'
  } else if (value === 'deal-autonomous-analysis-service') {
    return 'Autonomous Analysis Service'
  } else if (value === 'deal-vmc-analysis-service') {
    return 'VM&C Analysis Service'
  } else if (value === 'deal-icme-analysis-service') {
    return 'ICME Analysis Service'
  }
  return value
}
...
const fetchLeads = async (page = 1) => {
  try {
    ...
    let apiUrl = `/crm/record-child-get/${route.query.recordId}/Accounts-Deals?per_page=${perPage.value}&page=${page}&filters[c1_315._engineering_segment]=${getApiVariable(authStore.customerSuccessTeamSidebarIdentifier)}`

    if (selectedFilter.value === 'HTSS') {
      apiUrl += '&filters[deal_originated_by][]=2. HTSS&filters[deal_originated_by][]=2a. HTSS-2025&filters[deal_originated_by][]=2b. HTSS-2025Q1&filters[deal_originated_by][]=2c. HTSS-2025Q2&filters[deal_originated_by][]=2d. HTSS-2025Q3&filters[deal_originated_by][]=2e. HTSS-2025Q4&filters[deal_originated_by][]=2f. HTSS-2026&filters[deal_originated_by][]=2g. HTSS-2026Q1&filters[deal_originated_by][]=2h. HTSS-2026Q2&filters[deal_originated_by][]=2i. HTSS-2026Q3&filters[deal_originated_by][]=2j. HTSS-2026Q4&filters[deal_originated_by][]=2k. HTSS-2027&filters[deal_originated_by][]=2l. HTSS-2027Q1&filters[deal_originated_by][]=2m. HTSS-2027Q2&filters[deal_originated_by][]=2n. HTSS-2027Q3&filters[deal_originated_by][]=2o. HTSS-2027Q4&filters[deal_originated_by][]=2p. HTSS-2028&filters[deal_originated_by][]=2q. HTSS-2028Q1&filters[deal_originated_by][]=2r. HTSS-2028Q3&filters[deal_originated_by][]=3t. HTSS-3T028Q4'
    } else if (selectedFilter.value === 'ALL') {
      apiUrl += ''
    } else if (selectedFilter.value === 'MSC') {
      apiUrl += '&filters[deal_originated_by][]=1. MSC&filters[deal_originated_by][]=1a. MSC-Hexagon&filters[deal_originated_by][]=1b. MSC-Cadence'
    }
    ...
    <div class="container mx-auto">
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6">
        <h2 class="text-2xl font-bold text-gray-800 pb-2"> {{
            getHeadingText(authStore.customerSuccessTeamSidebarIdentifier) }} Deals - {{ route.query.name }}
          </h2>
        <!-- Header controls -->
        <div class="flex items-center justify-between mb-6 gap-4 flex-wrap">
          ...
          <div class="flex gap-2">
            <button @click="changeFilter('HTSS')" :class="[
            ...
            <button @click="changeFilter('MSC')" :class="[
              'px-4 py-2 rounded-lg font-medium transition-all duration-200',
              selectedFilter === 'MSC'
                ? 'bg-blue-600 text-white shadow-lg'
                : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            ...
            <button @click="changeFilter('ALL')" :class="[
              'px-4 py-2 rounded-lg font-medium transition-all duration-200',
              selectedFilter === 'ALL'
                ? 'bg-blue-600 text-white shadow-lg'
                : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
          ...
          <table class="min-w-full divide-y divide-gray-200 table-zebra">
            ...
            <tbody class="bg-white divide-y divide-gray-100">
              <tr v-if="isLoading" class="hover:bg-emerald-50 transition-colors">
                <td colspan="10" class="px-6  text-center text-gray-500">
              ...
              <tr v-else-if="leads.length === 0" class="hover:bg-emerald-50 transition-colors">
                <td colspan="10" class="px-2  text-center text-gray-500">
              ...
              <tr v-else v-for="(lead, index) in leads" :key="lead.id"
                ...
                <td class="px-6 py-0 whitespace-nowrap text-xs">
                  {{ index + 1 }}
                </td>
                <td v-for="field in fields" :key="field.id"
                  class="px-6 py-0 whitespace-nowrap text-sm text-gray-600 font-medium">
                  <router-link :to="{
        ...
        <div class="flex justify-between items-center mt-6">
          <p class="text-sm text-gray-600">
            Showing page <span class="font-semibold">{{ currentPage }}</span> of <span class="font-semibold">{{
              totalPages }} || Total Deals {{ totalAccounts }}</span>
          </p>
