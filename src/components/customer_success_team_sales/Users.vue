  ...
  <div class="min-h-screen w-full bg-gray-50">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
      <div class="mb-4 sm:mb-0">
        <h1 class="text-xl font-bold text-gray-900">{{route.query.name}} - User List</h1>
        <p class="text-gray-600 mt-2">Organization's users list</p>
      </div>
      <!-- Search -->
    ...
    <div class="overflow-x-auto bg-white rounded-2xl shadow-lg border border-gray-100">
      ...
      <table v-else class="min-w-full text-sm text-gray-700 table-zebra">
        <thead class="table-zebra bg-gradient-to-r from-blue-500 to-purple-600 text-white">
          <tr>
        ...
        <tbody :class="loading ? 'blur-sm' : ''">
          <tr v-for="(user, index) in users" :key="user.id"
            ...
            <td class="py-2 border-e px-4">{{ index + 1 }}</td>
            <td class="py-2 border-e px-4 font-semibold">
              <router-link :to="'/customer/Users/details/' + user.id" class="text-blue-600 text-sm font-semibold">
                {{ user.user.name }}</router-link>
            </td>
            <td class="py-2 border-e px-4">{{ user.user.email }}</td>
            <td class="py-2 border-e px-4 capitalize">{{ user.customer?.user.name }}</td>
            <td class="py-2 border-e px-4">
            <td class="px-6 py-2">
              <div class="flex flex-wrap gap-2" v-if="user.softwares?.length">
                <span v-for="software in user.softwares" :key="software.id"
                  class="bg-purple-100 text-purple-800 px-2 py-2 text-xs font-medium rounded-full">
                  {{ software.name }}
                </span>
...
const getUsers = async () => {
  ...
  const { data } = await api().get('end-users?customer_id=211&page=' + currentPage.value + '&per_page=20&with=softwares,user,customer.user&search=' + searchQuery.value)
