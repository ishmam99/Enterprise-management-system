...
const getApiVariable = (value) => {
  if (value === 'contact-structure-analysis-service') {
    return 'A. Structure Analysis Service' // ase
  } else if (value === 'contact-system-dynamics-analysis-service') {
    return 'B. System Dynamics Analysis Service'
  } else if (value === 'contact-acoustics-analysis-service') {
    return 'C. Acoustics Analysis Service' // ase
  } else if (value === 'contact-fluids-analysis-service') {
    return 'D. Fluids Analysis Service' // ase
  } else if (value === 'contact-autonomous-analysis-service') {
    return 'E. Autonomous Analysis Service'
  } else if (value === 'contact-vmc-analysis-service') {
    return 'F. VM&C Analysis Service' // ! ase
  } else if (value === 'contact-icme-analysis-service') {
    return 'G. ICME Analysis Service' // ase
  }
  return value
}
...
//  let apiUrl = `/crm/record-child-get/${route.query.recordId}/Accounts-Contacts?per_page=${perPage.value}&page=${page}`
    let apiUrl = `/crm/record-child-get/${route.query.recordId}/Accounts-Contacts?page=${page}&per_page=${perPage.value}&filters[c1_315._engineering_segment]=${getApiVariable(authStore.customerSuccessTeamSidebarIdentifier)}`
    ...
    <div class="container mx-auto">
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6">
       <h2 class="text-2xl font-bold text-gray-800 pb-2"> {{
            getHeadingText(authStore.customerSuccessTeamSidebarIdentifier) }} Contacts - {{ route.query.name }}
          </h2>
        <!-- Header controls -->
        ...
        <div v-else class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 table-zebra">
            <thead class="bg-gradient-to-r from-emerald-50 to-teal-50">
              <tr>
                <th class="px-6 py-4 border-x text-left text-xs font-bold text-emerald-700 uppercase tracking-wider">
                  #
                </th>
