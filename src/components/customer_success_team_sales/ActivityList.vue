    ...
    <div class="mx-auto max-w-7xl">
      <div class="md:flex md:items-center md:justify-between mb-10">
        <div class="flex-1 min-w-0">
          <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight sm:text-4xl">Activity Log</h1>
          <p class="mt-2 text-base text-slate-600">Track and manage customer success interactions.</p>
        </div>
        <div class="mt-6 flex md:ml-4 md:mt-0">
          <button @click="openCreateModal" class="inline-flex items-center px-6 py-3 border border-transparent shadow-sm text-sm font-semibold rounded-xl text-white bg-indigo-600 hover:bg-indigo-700 transition-all transform hover:scale-105">
            Log New Activity
          </button>
        </div>
      </div>

      <div v-if="!loading && activities.length > 0" class="bg-white shadow-xl shadow-slate-200/50 rounded-2xl border border-slate-200 overflow-hidden">
          ...
          <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
              <tr>
                ...
                <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest">Details</th>
                <th class="px-6 py-4 text-right text-xs font-bold text-slate-500 uppercase tracking-widest">Actions</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-200 bg-white">
              <tr v-for="act in activities" :key="act.id" class="hover:bg-indigo-50/30 transition-colors group">
                <td class="px-6 py-5 whitespace-nowrap text-sm font-medium text-slate-600">{{ formatDate(act.date) }}</td>
                <td class="px-6 py-5 whitespace-nowrap">
                  <span class="px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full border" :class="typeBadgeClass(act.type)">
                    {{ formatType(act.type) }}
                  </span>
                </td>
                ...
                <td class="px-6 py-5 whitespace-nowrap text-right text-sm font-bold">
                  <div class="flex justify-end space-x-4">
                    <button @click="openEditModal(act)" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                    <button @click="confirmDelete(act.id)" class="text-red-500 hover:text-red-700">Delete</button>
                  </div>
                </td>
              </tr>
            </tbody>
      ...
      <div v-if="showDeleteConfirm" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        <div class="fixed inset-0 bg-slate-900/75 transition-opacity" @click="showDeleteConfirm = false"></div>
        <div class="relative bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl">
          <div class="text-center">
            ...
            <p class="text-sm text-slate-500 mt-2">Are you sure you want to remove this activity? This action cannot be undone.</p>
          </div>
          <div class="mt-6 flex space-x-3">
            <button @click="showDeleteConfirm = false" class="flex-1 px-4 py-2 border border-slate-300 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-50">Cancel</button>
            <button @click="performDelete" :disabled="deleting" class="flex-1 px-4 py-2 bg-red-600 text-white rounded-xl text-sm font-bold hover:bg-red-700 disabled:opacity-50">
