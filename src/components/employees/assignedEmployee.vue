  ...
  <div class="min-h-screen w-full bg-gray-50 py-10 px-4">
    <div class=" mx-10">
      <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 class="text-2xl font-bold text-gray-800">Assigned employees</h1>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse table-zebra">
            <thead class="bg-sky-50 border-b border-gray-200">
              <tr>
                <th class="px-6 py-4 border-e text-xs font-semibold text-gray-600 uppercase tracking-wider">Employee & ID</th>
                <th class="px-6 py-4 border-e text-xs font-semibold text-gray-600 uppercase tracking-wider">Email</th>
                <th class="px-6 py-4 border-e text-xs font-semibold text-gray-600 uppercase tracking-wider">Joined Date</th>
                <th class="px-6 py-4 border-e text-xs font-semibold text-gray-600 uppercase tracking-wider">System Role</th>
                <th class="px-6 py-4 border-e text-xs font-semibold text-gray-600 uppercase tracking-wider text-right">Status</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
              ...
              <tr v-for="emp in employees" :key="emp.id" class="hover:bg-gray-50/50 transition">
                ...
                <td class="px-6 py-4 border-e text-sm text-gray-600">
                  {{ emp.email }}
                </td>
                <td class="px-6 py-4 border-e text-sm text-gray-600">
                ...
                <td class="px-6 py-4 border-e text-right">
...
const fetchEmployees = async () => {
  loading.value = true
  try {
    let url = `employees?with=user&department_id=${props.department_id}`
