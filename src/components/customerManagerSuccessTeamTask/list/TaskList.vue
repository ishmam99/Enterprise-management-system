...
const loadTasks = async () => {
  loading.value = true
  try {
    const res = await api().get(`/success-team-tasks?success_team_id=${authStore.team_id}&with=solution,software,user,assignedPerson,outputs,successTeam`)
    ...
    <div class="mx-auto max-w-7xl">
      ...
      <template v-else>
        ...
        <TaskGrid
          ...
          @view-details="(id) => router.push(`/customer_management/team-tasks/${id}`)"
        ...
        <TaskEmptyState
          ...
          @create-task="router.push('/customer_management_manager/team-tasks/create')"
          @reset-filters="resetFilters"
          @refresh="loadTasks"
        />
      </template>
