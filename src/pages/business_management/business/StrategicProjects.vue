    ...
    <div class="max-w-7xl mx-auto">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          ...
          <p class="text-gray-500 text-sm mt-1">Manage strategic initiatives and projects</p>
        </div>
        <div class="flex gap-2 mt-3 md:mt-0">
          <router-link to="/business_management/business/projects/create" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-xl text-sm font-semibold transition shadow-lg shadow-purple-200 flex items-center gap-2">
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="item in projects" :key="item.id" class="hover:bg-gray-50 transition">
                <td class="px-6 py-4"><span class="text-sm font-medium text-gray-900">{{ item.name }}</span></td>
                <td class="px-6 py-4"><span class="text-sm text-gray-600">{{ item.description || '-' }}</span></td>
                <td class="px-6 py-4">
                  <span class="text-xs px-2 py-1 rounded-full" :class="getStatusClass(item.status)">{{ item.status }}</span>
                </td>
                <td class="px-6 py-4 text-right">
                  <router-link :to="`/business_management/business/projects/edit/${item.id}`" class="text-blue-600 hover:text-blue-800 text-sm font-medium">Edit</router-link>
...
const projects = ref([
  { id: 1, name: 'Composite Materials Innovation', description: 'Developing next-gen carbon fiber composites', status: 'active' },
  { id: 2, name: 'European Market Expansion', description: 'Opening new distribution channels in Germany', status: 'planning' },
  { id: 3, name: 'Automation Factory Upgrade', description: 'Implementing robotic layup systems', status: 'review' },
  { id: 4, name: 'Medical Imaging Partnership', description: 'Collaboration with Siemens Healthineers', status: 'active' }
])
