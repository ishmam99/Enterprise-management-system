      ...
      <div class="bg-gradient-to-r from-emerald-500 via-green-500 to-teal-500 rounded-2xl shadow-xl p-6 w-full relative overflow-hidden">
        ...
        <div class="relative z-10 flex items-center justify-between">
          <div class="flex items-center space-x-4">
            ...
            <div>
              <h1 class="text-3xl font-bold text-white mb-1 drop-shadow-lg">
                Users List By Customer
              </h1>
              <p class="text-emerald-100 text-sm font-medium">
                Manage and view users organized by customer accounts
              </p>
            </div>
          </div>
          <div class="hidden sm:block bg-white bg-opacity-20 px-4 py-2 rounded-xl">
            ...
            <span class="text-emerald-100 text-sm ml-1">Total</span>
          </div>
        </div>
      </div>

      <!-- Search and Controls -->
      <div v-if="selectedCustomer" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <!-- Stats Cards -->
        ...
        <div class="flex flex-col sm:flex-row gap-4 items-center">
          ...
          <!-- Customer Select and Back Button -->
          <div class="flex gap-3">
            <button 
              @click="selectedCustomer = null; getUsers(false)"
            ...
            <select
              ...
              class="px-4 py-3 border-0 bg-white rounded-xl shadow-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all duration-300 text-gray-700"
        ...
        <table class="min-w-full text-sm text-gray-700">
          <thead class="bg-gradient-to-r from-emerald-600 to-green-600 text-white text-left">
            <tr>
              ...
              <th class="py-4 border-e border-emerald-500 px-6 font-semibold text-sm uppercase tracking-wider">Status</th>
              <th class="py-4 border-e border-emerald-500 px-6 font-semibold text-sm uppercase tracking-wider text-center">Actions</th>
            </tr>
          </thead>
          <tbody>
            ...
            >
              ...
              <td class="py-4 border-e px-6 text-gray-700">{{ req.customer?.user?.name }}</td>
      ...
      <div v-else>
        <table class="min-w-full text-sm text-gray-700">
          <thead class="bg-gradient-to-r from-emerald-600 to-green-600 text-white text-left">
            <tr>
              <th class="py-4 border-e border-emerald-500 px-6 font-semibold text-sm uppercase tracking-wider">#</th>
              <th class="py-4 border-e border-emerald-500 px-6 font-semibold text-sm uppercase tracking-wider">Customer Name</th>
              <th class="py-4 border-e border-emerald-500 px-6 font-semibold text-sm uppercase tracking-wider">Industry</th>
              <th class="py-4 border-e border-emerald-500 px-6 font-semibold text-sm uppercase tracking-wider">Users Count</th>
              <th class="py-4 border-e border-emerald-500 px-6 font-semibold text-sm uppercase tracking-wider text-center">Actions</th>
            </tr>
          </thead>
          <tbody>
            ...
            >
              ...
              <td class="py-4 border-e px-6 text-gray-600">
                <span class="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">
                  {{ req.customer?.industry?.name || 'N/A' }}
                </span>
              </td>
              <td class="py-4 border-e px-6">
              ...
              <td class="py-4 border-e px-6 text-center">
                <button 
                  @click="selectedCustomer = req.customer_id; getUsers(true)"
                  class="px-5 py-2.5 bg-gradient-to-r from-green-500 to-teal-500 text-white font-semibold rounded-xl hover:from-green-600 hover:to-teal-600 transition-all duration-300 transform hover:scale-105 shadow-lg flex items-center mx-auto"
                >
      ...
      <div v-if="users.length > 0 && meta" class="flex flex-col sm:flex-row justify-between items-center px-6 py-4 bg-gray-50 border-t border-gray-100">
        ...
        <div class="flex space-x-1">
          <button 
            ...
            @click="currentPage = link.page, getUsers(!!selectedCustomer)"
          >
...
const getUsers = async (type) => {
  ...
  if (selectedCustomer.value) {
    indStr = 'customer_id='+ selectedCustomer.value
  }
  if(type == false)
  {
    groupBy = '&group_by=customer_id'
  }
  else
   groupBy = `&per_page=20&page=${currentPage.value}&search=${encodeURIComponent(searchQuery.value)}&${indStr}`
  const { data } = await api().get(
    `end-users?with=user,customer.user,customer.industry${groupBy}`
  )
...
watch(searchQuery, debounce(() => {
  ...
  getUsers(!!selectedCustomer.value)
}, 500))
