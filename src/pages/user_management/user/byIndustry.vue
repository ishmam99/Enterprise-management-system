<template>
  <div class="p-6 min-h-screen w-4/5 bg-gray-50">
    ...
    <div class="flex flex-col gap-4 mb-6">
      <!-- Title Section -->
      <div class="bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 rounded-2xl shadow-xl p-6 w-full relative overflow-hidden">
        <!-- Background Pattern -->
        ...
        <div class="relative z-10 flex items-center justify-between">
          <div class="flex items-center space-x-4">
            ...
            <div>
              <h1 class="text-3xl font-bold text-white mb-1 drop-shadow-lg">
                Users List By Industry
              </h1>
              <p class="text-orange-100 text-sm font-medium">
                Manage and view users organized by industry sectors
              </p>
            </div>
          </div>
          <div class="hidden sm:block bg-white bg-opacity-20 px-4 py-2 rounded-xl">
            ...
            <span class="text-orange-100 text-sm ml-1">Total</span>
          </div>
        </div>
      </div>

      <!-- Search and Controls -->
      <div v-if="selectedIndustry" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <!-- Stats Cards -->
        <div class="flex space-x-3">
          <div class="bg-white px-4 py-2 rounded-xl shadow-md border border-gray-100">
            ...
            <span class="font-bold text-orange-600 ml-1">{{ users?.length || 0 }}</span>
          </div>
          <div class="bg-white px-4 py-2 rounded-xl shadow-md border border-gray-100">
            <span class="text-gray-600 text-sm">Page</span>
            <span class="font-bold text-orange-600 ml-1">{{ currentPage }}</span>
          </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-4 items-center">
          ...
          <div class="relative">
            <input
              ...
              class="pl-12 pr-4 py-3 w-full sm:w-80 border-0 bg-white rounded-xl shadow-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all duration-300 text-gray-700 placeholder-gray-400"
            />
            <i class="fa fa-search absolute left-4 top-3.5 text-orange-500"></i>
            <div class="absolute right-3 top-2.5">
              <div class="w-2 h-2 bg-orange-400 rounded-full animate-ping"></div>
            </div>
          </div>

          <!-- Industry Select and Back Button -->
          <div class="flex gap-3">
            <button 
              @click="selectedIndustry = null; getUsers(false)"
            ...
            <select
              ...
              class="px-4 py-3 border-0 bg-white rounded-xl shadow-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all duration-300 text-gray-700"
      ...
      <div v-else class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex space-x-3">
          <div class="bg-white px-4 py-2 rounded-xl shadow-md border border-gray-100">
            <span class="text-gray-600 text-sm">Industries</span>
            <span class="font-bold text-orange-600 ml-1">{{ users?.length || 0 }}</span>
          </div>
          <div class="bg-white px-4 py-2 rounded-xl shadow-md border border-gray-100">
            <span class="text-gray-600 text-sm">Page</span>
    ...
    <div class="overflow-x-auto bg-white rounded-2xl shadow-xl border border-gray-100">
      ...
      <div v-if="selectedIndustry">
        <table class="min-w-full text-sm text-gray-700">
          <thead class="bg-gradient-to-r from-orange-600 to-amber-600 text-white text-left">
            <tr>
              <th class="py-4 border-e border-orange-500 px-6 font-semibold text-sm uppercase tracking-wider">#</th>
              <th class="py-4 border-e border-orange-500 px-6 font-semibold text-sm uppercase tracking-wider">Name</th>
              <th class="py-4 border-e border-orange-500 px-6 font-semibold text-sm uppercase tracking-wider">Email</th>
              <th class="py-4 border-e border-orange-500 px-6 font-semibold text-sm uppercase tracking-wider">Industry</th>
              <th class="py-4 border-e border-orange-500 px-6 font-semibold text-sm uppercase tracking-wider">Status</th>
              <th class="py-4 border-e border-orange-500 px-6 font-semibold text-sm uppercase tracking-wider text-center">Actions</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr 
              ...
              :key="req.id"
              class="border-b border-gray-100 hover:bg-gradient-to-r hover:from-orange-50 hover:to-amber-50 transition-all duration-200"
            >
              ...
              <td class="py-4 border-e px-6">
                <span class="px-3 py-1.5 bg-gradient-to-r from-orange-100 to-amber-100 text-orange-700 rounded-full text-xs font-semibold border border-orange-200">
                  {{ req.industry?.name }}
                </span>
              </td>
              <td class="py-4 border-e px-6">
      ...
      <div v-else>
        <table class="min-w-full text-sm text-gray-700">
          <thead class="bg-gradient-to-r from-orange-600 to-amber-600 text-white text-left">
            <tr>
              <th class="py-4 border-e border-orange-500 px-6 font-semibold text-sm uppercase tracking-wider">#</th>
              <th class="py-4 border-e border-orange-500 px-6 font-semibold text-sm uppercase tracking-wider">Industry Name</th>
              <th class="py-4 border-e border-orange-500 px-6 font-semibold text-sm uppercase tracking-wider">Users Count</th>
              <th class="py-4 border-e border-orange-500 px-6 font-semibold text-sm uppercase tracking-wider text-center">Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="users.length === 0">
              <td colspan="4" class="text-center py-12 text-gray-400">
                <div class="flex flex-col items-center">
                  <span class="text-4xl mb-2">🏭</span>
                  <p class="text-lg">No industries found</p>
                  <p class="text-sm text-gray-500 mt-1">Try adjusting your search criteria</p>
                </div>
              </td>
            </tr>
            ...
            >
              ...
              <td class="py-4 border-e px-6">
                <span class="px-3 py-1.5 bg-gradient-to-r from-orange-100 to-amber-100 text-orange-700 rounded-full text-sm font-semibold border border-orange-200">
                  {{ req.total }} users
                </span>
              </td>
              <td class="py-4 border-e px-6 text-center">
                <button 
                  @click="selectedIndustry = req.industry_id; getUsers(true)"
                  class="px-5 py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold rounded-xl hover:from-amber-600 hover:to-orange-600 transition-all duration-300 transform hover:scale-105 shadow-lg flex items-center mx-auto"
                >
      ...
      <div v-if="users.length > 0 && meta" class="flex flex-col sm:flex-row justify-between items-center px-6 py-4 bg-gray-50 border-t border-gray-100">
        ...
        <div class="flex space-x-1">
          <button 
            ...
            @click="currentPage = link.page, getUsers(!!selectedIndustry)"
          >
...
const getUsers = async (type) => {
  ...
  if (selectedIndustry.value) {
    indStr = '&industry_id='+ selectedIndustry.value
  }
  if(type == false)
  {
  ...
  const { data } = await api().get(
    `end-users?with=industry,user${groupBy}${indStr}`
  )
...
watch(searchQuery, debounce(() => {
  ...
  getUsers(!!selectedIndustry.value)
}, 500))
