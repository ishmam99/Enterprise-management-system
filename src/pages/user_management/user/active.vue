          ...
          <div class="flex items-center space-x-4">
            ...
            <div>
              ...
              <p class="text-emerald-100 text-sm font-medium">
                Manage and monitor all active user accounts
              </p>
            </div>
          </div>
          ...
          >
            ...
            <td class="py-4 border-e px-6 font-semibold">
              ...
              >
                ...
                {{ user.user?.name || (user.first_name && user.last_name ? user.first_name + ' ' + user.last_name : '') }}
              </router-link>
            </td>
            <td class="py-4 border-e px-6 text-gray-600">{{ user.user?.email  || user.email}}</td>
            <td class="py-4 border-e px-6 capitalize text-gray-700">{{ user.customer?.user.name || 'Independent' }}</td>
            <td class="py-4 border-e px-6 text-right">
              <button
                v-if="user.status == 1 || user.status == 'active'"
                class="px-5 py-2.5 rounded-xl shadow-md bg-gradient-to-r from-red-500 to-pink-600 text-white text-sm font-semibold hover:from-red-600 hover:to-pink-700 transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
              ...
              >
                ...
                {{ actionLoading && approvingId === user.id ? 'Processing...' : 'Make Inactive' }}
              </button>
            </td>
          </tr>
...
const getUsers = async()=>{
  loading.value = true
  const {data} = await api().get('end-users?page='+currentPage.value+'&per_page=20&with=user,customer.user&status=1&search='+searchQuery.value)
