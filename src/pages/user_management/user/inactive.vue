<template>
  <div class="p-6 min-h-screen w-4/5 bg-gray-50">
    ...
    <div class="flex flex-col gap-4 mb-6">
      <!-- Title Section -->
      <div class="bg-gradient-to-r from-red-500 via-rose-500 to-pink-500 rounded-2xl shadow-xl p-6 w-full relative overflow-hidden">
        <!-- Background Pattern -->
        ...
        <div class="relative z-10 flex items-center justify-between">
          <div class="flex items-center space-x-4">
            ...
            <div>
              ...
              <p class="text-red-100 text-sm font-medium">
                Manage and monitor all inactive user accounts
              </p>
            </div>
          </div>
          <div class="hidden sm:block bg-white bg-opacity-20 px-4 py-2 rounded-xl">
            ...
            <span class="text-red-100 text-sm ml-1">Users</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex space-x-3">
          <div class="bg-white px-4 py-2 rounded-xl shadow-md border border-gray-100">
            ...
            <span class="font-bold text-red-600 ml-1">{{ users?.length || 0 }}</span>
          </div>
          <div class="bg-white px-4 py-2 rounded-xl shadow-md border border-gray-100">
            <span class="text-gray-600 text-sm">Page</span>
            <span class="font-bold text-red-600 ml-1">{{ currentPage }}</span>
          </div>
        </div>
        ...
        <div class="relative">
          <input
            ...
            class="pl-12 pr-4 py-3 w-full sm:w-80 border-0 bg-white rounded-xl shadow-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition-all duration-300 text-gray-700 placeholder-gray-400"
          />
          <i class="fa fa-search absolute left-4 top-3.5 text-red-500"></i>
    ...
    <div class="overflow-x-auto bg-white rounded-2xl shadow-xl border border-gray-100">
      ...
      <table v-else class="min-w-full text-sm text-gray-700">
        <thead class="bg-gradient-to-r from-red-600 to-rose-600 text-white text-left">
          <tr>
            <th class="py-4 border-e border-red-500 px-6 font-semibold text-sm uppercase tracking-wider">#</th>
            <th class="py-4 border-e border-red-500 px-6 font-semibold text-sm uppercase tracking-wider">Name</th>
            <th class="py-4 border-e border-red-500 px-6 font-semibold text-sm uppercase tracking-wider">Email</th>
            <th class="py-4 border-e border-red-500 px-6 font-semibold text-sm uppercase tracking-wider">Customer</th>
            <th class="py-4 border-e border-red-500 px-6 font-semibold text-sm uppercase tracking-wider text-right">Action</th>
          </tr>
        </thead>
        <tbody :class="loading?'blur-sm':''">
          <tr
            ...
            :key="user.id"
            class="border-b border-gray-100 hover:bg-gradient-to-r hover:from-red-50 hover:to-pink-50 transition-all duration-200"
          >
            ...
            <td class="py-4 border-e px-6 font-semibold">
              <router-link 
                ...
                class="text-red-700 hover:text-red-900 transition-colors duration-200 flex items-center"
              >
            ...
            <td class="py-4 border-e px-6 text-right">
              <button
                v-if="user.status == 2 || user.status == 'inactive'"
              ...
              >
                ...
                {{ actionLoading && approvingId === user.id ? 'Activating...' : 'Activate' }}
              </button>
            </td>
          </tr>
          <tr v-if="users?.length === 0">
            <td colspan="5" class="text-center py-12 text-gray-400">
              <div class="flex flex-col items-center">
                ...
                <p class="text-lg">No inactive users found</p>
...
const getUsers = async()=>{
  loading.value = true
  const {data} = await api().get('end-users?page='+currentPage.value+'&per_page=20&with=user,customer.user&status=2&search='+searchQuery.value)
...
const statusUpdate = async (user) => {
  ...
  try {
    ...
    Swal.fire({
      title: 'Success!',
      text: `${user.user.name} has been activated successfully.`,
      icon: 'success',
      timer: 2000,
