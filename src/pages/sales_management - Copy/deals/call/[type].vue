...
const setDate = () => {
  ...
  switch (type) {
    ...
    case "nextWeek":
      startDate.value = moment().add(1, "week").startOf("week").format("YYYY-MM-DD");
      endDate.value = moment().add(1, "week").endOf("week").format("YYYY-MM-DD");
      break;

    case "lastWeek":
      startDate.value = moment().subtract(1, "week").startOf("week").format("YYYY-MM-DD");
      endDate.value = moment().subtract(1, "week").endOf("week").format("YYYY-MM-DD");
      break;
    ...
    case "nextMonth":
      startDate.value = moment().add(1, "month").startOf("month").format("YYYY-MM-DD");
      endDate.value = moment().add(1, "month").endOf("month").format("YYYY-MM-DD");
      break;

    case "lastMonth":
      startDate.value = moment().subtract(1, "month").startOf("month").format("YYYY-MM-DD");
      endDate.value = moment().subtract(1, "month").endOf("month").format("YYYY-MM-DD");
      break;

    // --- Quarters ---
    case "thisQuarter":
      startDate.value = moment().startOf("quarter").format("YYYY-MM-DD");
      endDate.value = moment().endOf("quarter").format("YYYY-MM-DD");
      break;

    case "nextQuarter":
      startDate.value = moment().add(1, "quarter").startOf("quarter").format("YYYY-MM-DD");
      endDate.value = moment().add(1, "quarter").endOf("quarter").format("YYYY-MM-DD");
      break;

    case "lastQuarter":
      startDate.value = moment().subtract(1, "quarter").startOf("quarter").format("YYYY-MM-DD");
      endDate.value = moment().subtract(1, "quarter").endOf("quarter").format("YYYY-MM-DD");
      break;
    ...
    case "nextYear":
      startDate.value = moment().add(1, "year").startOf("year").format("YYYY-MM-DD");
      endDate.value = moment().add(1, "year").endOf("year").format("YYYY-MM-DD");
      break;

    case "lastYear":
      startDate.value = moment().subtract(1, "year").startOf("year").format("YYYY-MM-DD");
      endDate.value = moment().subtract(1, "year").endOf("year").format("YYYY-MM-DD");
      break;
...
const fetchDeals = async (page = 1) => {
  try {
    ...
    const { data } = await api().get(
      `/crm/modules/5/records?date_field=call_schedule_date&start_date=${startDate.value}&end_date=${endDate.value}&page=${page}&per_page=${perPage.value}`
    )

    deals.value = data.data
    totalPages.value = data.last_page
    totalDeals.value = data.total
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
          <div class="flex flex-col gap-1">
            ...
            <p class="text-sm text-gray-600">
              ...
              <span class="font-bold">{{ totalDeals }}</span>
            </p>
            <p class="text-sm text-gray-600">
        ...
        <div v-else class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 table-zebra">
            <thead class="bg-gradient-to-r from-emerald-50 to-teal-50">
              <tr>
                <th class="px-6 py-2 border-x text-left text-xs font-bold text-emerald-700 uppercase tracking-wider">
                  #
                </th>
            ...
            <tbody class="bg-white divide-y divide-gray-100">
              ...
              <tr v-else-if="deals?.length === 0" class="hover:bg-emerald-50 transition-colors">
              ...
              <tr v-else v-for="(deal, index) in deals" :key="deal.id"
                ...
                <td v-for="field in fields" :key="field.id"
                  ...
                  <router-link :to="{
                    ...
                    {{deal.values.find((e) => e.field_id == field.id)?.value}}
                  </router-link>
