  ...
  <div class="w-11/12 mx-auto pt-6 px-10 space-y-4 min-h-screen">
    ...
    >
      <div class="flex items-center gap-4">
        ...
        <div>
          <h2 class="text-2xl font-semibold text-white">Monthly Timecard Entry</h2>
...
// const isToday = (date) => {
//   return attendanceByDate[date]?.length
// }
// const isToday = () => true
...
// const selectedProduct = (date) => {
//   const f = getRowForm(date)
//   return productList.value.find((p) => p.id == f.product_id)
// }
...
const loadAttendanceForWeek = async () => {
  ...
  const start = formatLocalDate(weekStart.value)
  const endDate = new Date(weekStart.value.getFullYear(), weekStart.value.getMonth() + 1, 0)
  ...
  // const product = productList.value.find(p => p.id == task.product_id)
  // const module = product?.modules?.find(m => m.id == task.module_id)
...
/** * Helper function to handle the logic for each date
 * Optimized to extract IDs directly from the API response
 */
const processAttendanceResponse = (date, data) => {
  if (data && data.length) {
    const attendance = data[0]
    const attendanceId = attendance.id // This is your attendance_id
    ...
    /* ================= DYNAMIC ID TRACKING ================= */
    // Find the log that hasn't been logged out yet
    const activeSession = attendance.attendance_logs.find((log) => !log.logout_time)

    if (activeSession) {
      isLoggedInByDate[date] = true
      // Store the specific IDs for this date in a local reactive object if needed,
      // or simply rely on the entries structure.
      if (date === todayISO) {
        // We still update these for the current session, but from the API data
        authStore.attendance_info_id = activeSession.id
        authStore.login_id = attendanceId
      }
    } else {
      isLoggedInByDate[date] = false
      if (date === todayISO) {
        authStore.attendance_info_id = null
        // Keep login_id so the Save button still works if they logged out but are still on the page
        authStore.login_id = attendanceId
      }
    }
...
const totalTimeByDate = computed(() => {
  ...
  for (const [date, times] of Object.entries(timesByDate.value)) {
    ...
    times.forEach((t) => {
      // Use t.hour and t.minute instead of total_hours / total_minute
...
// const weekStart = ref(getSaturday(new Date(todayISO)))
const weekOffset = ref(0)
const weekStart = computed(() => {
  const d = new Date(todayISO)
  d.setMonth(d.getMonth() + weekOffset.value) // use month offset
...
const changeWeek = async (dir) => {
  weekOffset.value += dir // now moves month by month
  await loadAttendanceForWeek() // still works, fetches full month
}
