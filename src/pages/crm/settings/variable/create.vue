    ...
    <div class="max-w-5xl mx-auto bg-white rounded-2xl shadow-lg p-8">
      ...
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-gray-800">
          Create CRM Module Field
        </h1>
        <button
          class="bg-indigo-600 flex gap-3 text-white px-5 py-2 rounded-xl hover:bg-indigo-700 transition disabled:opacity-50"
          :disabled="saving"
          @click="saveField"
        >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div>
          <label class="block text-sm font-medium text-gray-600 mb-1">Select Module <span class="text-red-500">*</span></label>
          <select
            v-model="field.module_id"
        ...
        <div>
          <label class="block text-sm font-medium text-gray-600 mb-1">Field Label <span class="text-red-500">*</span></label>
          <input
            v-model="field.label"
            type="text"
            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-400"
        ...
        <div>
          <label class="block text-sm font-medium text-gray-600 mb-1">Field Type <span class="text-red-500">*</span></label>
          <select
            v-model="field.type"
            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-400"
            required
        ...
        <div class="flex w-full justify-start gap-10 items-center">
          ...
          <div class="flex items-center mt-6">
            <input
              ...
              class="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
            />
            <label for="unique" class="ml-2 text-sm text-gray-700">Unique</label>
          </div>
        </div>
      </div>

      <!-- Dropdown / Checkbox Values Section -->
      <div v-if="field.type === 'select' || field.type === 'checkbox'" class="mb-8">
      ...
      <div v-if="message" :class="['mt-4 p-3 rounded-lg', messageType === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700']">
        {{ message }}
      </div>
    </div>
  </div>
...
// Parse bulk options text into array
...
const saveField = async() => {
  ...
  try {
    ...
    const dataToSend = {
      module_id: field.value.module_id,
      label: field.value.label,
      name: fieldName,
      type: field.value.type,
      order_group: field.value.order_group || null,
      required: field.value.required ? 1 : 0,
      unique: field.value.unique ? 1 : 0,
      options: field.value.options
    };

    await api().post(`/crm/field`, dataToSend);
