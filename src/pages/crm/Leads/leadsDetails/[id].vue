        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex gap-2 flex-row-reverse">
            ...
            <div class="flex flex-col gap-2">
              ...
              >
                <span class="font-medium">Update Assigned Executive</span>
              </button>
              <button
                v-else
                class="flex items-center bg-emerald-700 text-white hover:bg-emerald-500 rounded-lg px-4 py-2 transition-all duration-300"
              ...
              >
                <span class="font-medium">Assign Executive</span>
              </button>
            </div>
            ...
            <div >
              <button
                ...
                @click="updateAssignModal('sales-manager')"
              >
                <span class="font-medium">Update Assigned Manager</span>
              </button>
              <button
                v-else
                class="flex items-center bg-sky-700 text-white hover:bg-sky-500 rounded-lg px-4 py-2 transition-all duration-300"
                title="Assign Manager"
                @click="openAssignModal('sales-manager')"
              >
          ...
          >
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
              <h2 class="text-xl font-semibold mb-4 text-gray-800">
                Assign {{ assignRole === 'sales-manager' ? 'Sales-Manager' : 'Sales-Executive' }}
              </h2>

              <div v-if="isLoading" class="text-center py-6 text-gray-500">
                Loading {{ assignRole }}s...
              </div>
              ...
              <div v-else>
                ...
                >
                  ...
                  <option
                    v-for="user in assignRole === 'sales-manager' ? managerData : executiveData"
                    :key="user.id"
                    :value="user.id"
                  >
          ...
          >
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
              <h2 class="text-xl font-semibold mb-4 text-gray-800">
                Update {{ assignRole === 'sales-manager' ? 'Sales-Manager' : 'Sales-Executive' }}
              </h2>
      ...
      >
         <h2 class="text-xl font-semibold mb-4 flex items-center text-gray-800">
              <div class="bg-blue-100 p-2 rounded-lg mr-3">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><path d="M13 9H21L11 24V15H4L13 0V9ZM11 11V7.22063L7.53238 13H13V17.3944L17.263 11H11Z"></path></svg>
              </div>
              B. Account: 0_Rapid Actions
            </h2>
        ...
        <div  class="space-y-4 text-sm">
          <div class="bg-white rounded-b-xl shadow-md border border-gray-200 p-6">
            ...
            <div v-else class="grid md:grid-cols-5 gap-6">
              ...
              <div
                v-for="field in emptyBottomGroup"
                :key="field.id"
                class="flex flex-col bg-white/40 border border-slate-200 rounded-xl px-5 py-3 shadow-sm hover:shadow-md transition-all"
              >
                <template v-if="editingEmptyFieldId !== field.id">
...
const managerName = computed(() => {
  const manager = getAssignments.value.find((a) => a.role === 'sales-manager')
...
const assignUser = async () => {
  ...
  try {
    const payload = {
      ...
      permission_level: assignRole.value === 'sales-manager' ? 'edit' : 'view'
    }
    await api().post(`/crm/assign-record/${route.params.id}`, payload)

    showToast(
      `${
        assignRole.value === 'sales-manager' ? 'Sales-Manager' : 'Sales-Executive'
