...
const fetchLeads = async (page = 1) => {
  try {
    ...
    if (selectedView.value === 'all') {
      ...
      const { data } = await api().get(
        `/crm/modules/1/records?page=${page}&per_page=${perPage.value}&custom_view_id=${selectedView.value.id}`
      )
      leads.value = data.data
      currentPage.value = data.current_page
      totalPages.value = data.last_page
...
const submitImport = async () => {
  ...
  try {
    ...
    await api().post('/crm/modules/1/import/excel', formData)
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
          <div class="flex gap-2 items-center">
            <h2 class="text-2xl font-bold text-gray-800 mb-1">Leads</h2>
            <select
            ...
            <div class="flex items-center gap-2">
              ...
              <select
                id="perPage"
                v-model="perPage"
                class="border rounded pe-7 py-1 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none"
              >
...
>
  <div class="bg-white w-full max-w-md rounded-xl shadow-2xl animate-fadeIn">
    ...
    <!-- Header -->
    <div class="px-6 py-4 border-b flex justify-between items-center">
      <h2 class="text-xl font-semibold text-gray-800">Import Leads from Excel File</h2>
