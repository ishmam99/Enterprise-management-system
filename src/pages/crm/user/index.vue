  ...
  <div class="sales-management-dashboard p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-6 mb-8">
      <div class="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-2xl shadow-lg border border-blue-200 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
        <div class="flex items-center gap-3 mb-4">
          ...
          <h3 class="text-blue-700 text-sm font-semibold">Total CRM Executives</h3>
        </div>
        <p class="text-3xl font-bold text-blue-800 mb-2">
          <span class="text-blue-600 font-bold flex gap-1">
            <count-up :end-val="statsData.crmExecutives"></count-up> +
          </span>
        </p>
      ...
      <div class="bg-gradient-to-br from-emerald-50 to-emerald-100 p-6 rounded-2xl shadow-lg border border-emerald-200 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
        <div class="flex items-center gap-3 mb-4">
          ...
          <h3 class="text-emerald-700 text-sm font-semibold">Total CRM Managers</h3>
        </div>
        <p class="text-3xl font-bold text-emerald-800 mb-2">
          <span class="text-emerald-600 font-bold flex gap-1">
            <count-up :end-val="statsData.crmManagers"></count-up> +
          </span>
        </p>
      ...
      <div class="bg-gradient-to-br from-cyan-50 to-cyan-100 p-6 rounded-2xl shadow-lg border border-cyan-200 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
        <div class="flex items-center gap-3 mb-4">
          ...
          <h3 class="text-cyan-700 text-sm font-semibold">Total Customers</h3>
        </div>
        <p class="text-3xl font-bold text-cyan-800 mb-2">
          <span class="text-cyan-600 font-bold flex gap-1">
            <count-up :end-val="statsData.customers ? statsData.customers : '22'"></count-up> +
          </span>
        </p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        ...
        <div class="flex items-center gap-3 mb-4">
          ...
          <h3 class="text-sky-700 text-sm font-semibold">Total Sales Executives</h3>
        </div>
        <p class="text-3xl font-bold text-sky-800 mb-2">
          <span class="text-sky-600 font-bold flex gap-1">
            <count-up :end-val="statsData.salesExecutives"></count-up> +
          </span>
        </p>
        <div class="w-full bg-sky-200 rounded-full h-2">
          <div class="bg-gradient-to-r from-sky-500 to-sky-600 h-2 rounded-full" style="width: 75%"></div>
        </div>
      </div>
      ...
      <div class="bg-gradient-to-br from-emerald-50 to-emerald-100 p-6 rounded-2xl shadow-lg border border-emerald-200 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
        <div class="flex items-center gap-3 mb-4">
          ...
          <h3 class="text-emerald-700 text-sm font-semibold">Total Sales Managers</h3>
        </div>
        <p class="text-3xl font-bold text-emerald-800 mb-2">
          <span class="text-emerald-600 font-bold flex gap-1">
            <count-up :end-val="statsData.salesManagers"></count-up> +
          </span>
        </p>
        <div class="w-full bg-emerald-200 rounded-full h-2">
          <div class="bg-gradient-to-r from-emerald-500 to-emerald-600 h-2 rounded-full" style="width: 85%"></div>
...
const fetchStats = async () => {
  const { data } = await api().get('/users-role-wise-count')

  let total = 0

  data.data.forEach(item => {
    const count = Number(item.total_users)
    total += count

    switch (item.role) {
      case 'customer':
        statsData.value.customers = count
        break
      case 'crm-executive':
        statsData.value.crmExecutives = count
        break
      case 'crm-manager':
        statsData.value.crmManagers = count
        break
      case 'crm-director':
        statsData.value.crmDirectors = count
        break
      case 'sales-executive':
        statsData.value.salesExecutives = count
        break
      case 'sales-manager':
        statsData.value.salesManagers = count
        break
    }
  })

  statsData.value.totalUsers = total
}
