...
const submitImport = async () => {
  ...
  try {
    ...
    await api().post('/crm/modules/5/import/excel', formData)
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
          <div class="flex gap-2 items-center">
            <h2 class="text-2xl font-bold text-gray-800 mb-1">Deals</h2>
          ...
          <div class="flex items-center gap-2">
              ...
              <!-- <router-link
                ...
                <button  class=" bg-indigo-600 rounded text-sm px-2 btn-sm py-1 font-semibold hover:bg-indigo-700 text-white" @click="openImportModal">
                Import from Excel
              </button>
              <button
  class="bg-sky-600 px-4 rounded text-sm py-1.5 font-semibold hover:bg-sky-700 text-white disabled:opacity-50 disabled:cursor-not-allowed"
...
>
  <div class="bg-white w-full max-w-lg rounded-xl shadow-2xl animate-fadeIn">
    ...
    <div class="px-6 py-5 space-y-4">
      <label class="text-sm font-medium text-gray-700">Select {{ selectedAssignRole === 'manager' ? 'Manager' : 'Executive' }}</label>

      <select
        v-model="assignUserId"
        class="w-full border px-3 py-2 rounded-lg focus:ring-2 focus:ring-indigo-500"
      >
        <option value="" disabled>Select {{ selectedAssignRole === 'manager' ? 'Manager' : 'Executive' }}</option>
  ...
  />
                ...
                <div class="flex flex-col gap-1 p-1">
                  <span class="border px-2 rounded border-emerald-500 text-emerald-500"> Manager: {{ getManagerName(lead) || 'Not Assigned' }}</span>
                 <span class="border px-2 rounded text-violet-500 border-violet-500">Executive: {{ getExecutiveName(lead) || 'Not Assigned' }}</span>
                </div>
                </td> -->
                <!-- <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 font-medium">
        ...
        <div class="flex justify-between items-center mt-6">
          <p class="text-sm text-gray-600">
            Showing page <span class="font-semibold">{{ currentPage }}</span> of <span class="font-semibold">{{ totalPages }}  ||  Total Deals: {{ totalAccounts }}</span>
          </p>
...
>
  <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg animate-fadeIn">
    ...
    <div class="px-6 py-4 border-b flex justify-between items-center">
      ...
      <button @click="closeMassUpdate" class="text-gray-500 hover:text-gray-700 text-xl">&times;</button>
    </div>
    ...
    <div class="px-6 py-4 border-t flex justify-end space-x-3 bg-gray-50 rounded-b-xl">
      <button @click="closeMassUpdate" class="bg-red-500 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium">Cancel</button>
<button
    ...
    Updating…
        ...
        >
          <div class="bg-white w-full max-w-md rounded-xl shadow-2xl animate-fadeIn">
            <!-- Header -->
            <div class="px-6 py-4 border-b flex justify-between items-center">
              <h2 class="text-xl font-semibold text-gray-800">Import Deals from Excel File</h2>
