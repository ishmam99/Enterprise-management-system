        ...
        <tbody>
          ...
          <tr v-for="(req, index) in customers" :key="req.id"
            ...
            <td class="py-3 px-4 text-center flex justify-center gap-3">
              ...
              <router-link v-if="req.id" :to="{name:'customer_management-id',params:{id:req.id},query:req}" class="px-3 py-1 text-white btn btn-info rounded-lg hover:bg-blue-100 transition" @click="viewDetails(req)">
                View
              </router-link>
            </td>
          </tr>
