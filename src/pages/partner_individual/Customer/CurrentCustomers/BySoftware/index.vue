  ...
  <div class="p-6 w-4/5 bg-gray-50 min-h-screen mx-auto">
    ...
    <div v-if="selectedSoftware" class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
      <div class="relative w-full sm:w-1/3 mb-4 sm:mb-0">
        <input v-model="searchQuery" type="text" placeholder="Search by user..."
      ...
      <div class="flex justify-between gap-3 items-center">
        ...
        <button class="btn btn-warning" @click="selectedSoftware = null, getCustomers(false)">
          <i class="ri-arrow-left-double-fill"></i>
          Back to Software List</button>
        <select v-model="selectedSoftware"
          ...
          <option v-for="software in softwares" :key="software" :value="software.name">{{ software.name }}</option>
    ...
    <div v-if="selectedSoftware" class="overflow-x-auto bg-white shadow-md rounded-xl border border-gray-100">
      ...
      <table class="min-w-full text-sm text-gray-700 table-zebra">
        ...
        <tbody>
          ...
          <tr v-for="(req, index) in customers" :key="req.id"
            ...
            <td class="py-3 border-e px-4">{{ selectedSoftware }}</td>
            ...
            <td class="py-3 px-4 text-center flex justify-center gap-3">
              ...
              <router-link v-if="req.id" :to="'/partner/Customer/CustomerDetails/' + req.id"
                class="px-3 py-1 text-white btn btn-info rounded-lg hover:bg-blue-100 transition">
                View
              </router-link>
              <router-link v-if="req.id" :to="{
                path: `/partner/Customer/ActivityList/${req.id}`,
      ...
      <div v-if="customers.length > 0 && meta" class="flex px-4 py-2 justify-between items-center mt-6">
        ...
        <div class=" bg-cyan-500 space-x-1 px-2 rounded-md py-1">
          ...
          <button v-for="link in meta.links" :key="link.label" class="p-3 btn btn-info rounded-md"
            :class="currentPage == link.page ? 'bg-blue-500 text-white' : 'bg-white text-blue-800'"
            @click="currentPage = link.page, getCustomers()">
      ...
      <table class="min-w-full text-sm text-gray-700 table-zebra">
        ...
        <tbody>
          <tr v-for="(req, index) in customers" :key="req.id"
            class="border-b border-gray-100 hover:bg-gray-50 transition">
            <td class="py-3 border-e px-4">{{ index + 1 }}</td>
            <td class="py-3 border-e px-4 font-semibold">{{ req.name }}</td>
            <td class="py-3 border-e px-4">{{ req.total }}</td>
            <td class="py-3 border-e px-4"><button class="btn btn-info text-white"
                @click="selectedSoftware = req.name, getCustomers(true)">View List</button></td>
...
const getCustomers = async (type) => {
  ...
  customers.value = data.data.data || data.data.filter(e => e.name != null)
