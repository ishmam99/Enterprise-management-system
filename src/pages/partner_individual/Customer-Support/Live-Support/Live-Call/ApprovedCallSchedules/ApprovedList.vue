<template>
  <div class="min-h-screen bg-gradient-to-br from-green-50 via-blue-50 to-purple-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="bg-white rounded-2xl p-6 shadow-lg border border-green-200">
          <div class="flex items-center">
            ...
            <div>
              <p class="text-2xl font-bold text-gray-900">{{ stats.totalCalls }}</p>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-lg border border-blue-200">
          <div class="flex items-center">
            <div class="w-12 h-12 bg-gradient-to-r from-blue-100 to-cyan-100 rounded-xl flex items-center justify-center mr-4">
              <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
              </svg>
            </div>
            <div>
              <p class="text-2xl font-bold text-gray-900">{{ stats.upcomingCalls }}</p>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-lg border border-purple-200">
          <div class="flex items-center">
            ...
            <div>
              <p class="text-2xl font-bold text-gray-900">{{ stats.liveNow }}</p>
              <p class="text-sm text-gray-600">Live Now</p>
            </div>
          </div>
        </div>
      ...
      <!-- Calls Grid -->
      <div v-else-if="filteredCalls.length > 0" class="space-y-6">
        <div
        ...
        >
          ...
          <div class="p-6 border-b border-gray-100">
            <div class="flex justify-between items-start mb-3">
              <div>
                <h3 class="text-lg font-semibold text-gray-900 line-clamp-1">{{ call.title || 'Untitled Call' }}</h3>
                <p class="text-sm text-gray-500 mt-1">Call #{{ call.id }}</p>
              </div>
              <span
                :class="[
            ...
            <p class="text-gray-600 text-sm line-clamp-2">{{ call.description || 'No description provided' }}</p>
          </div>
          ...
          <div class="p-6 space-y-4">
            ...
            <div class="flex items-center text-sm text-gray-600">
              ...
              <span>{{ formatTime(call.start_time) }} - {{ formatTime(call.end_time) }}</span>
            </div>
            ...
            <div class="flex items-center text-sm text-gray-600">
              ...
              <span class="capitalize">{{ (call.call_type || call.type)?.replace('-', ' ') || 'N/A' }}</span>
            </div>
            ...
            <div class="flex items-center text-sm">
              ...
              <span
                :class="[
                  ...
                  call.priority === 'urgent' ? 'text-red-600' :
                  call.priority === 'high' ? 'text-orange-600' :
                  call.priority === 'medium' ? 'text-yellow-600' :
                  'text-green-600'
                ]"
              >
                {{ call.priority || 'normal' }} priority
              </span>
            </div>
          </div>
          ...
          <div class="p-6 border-t border-gray-100 bg-gray-50 rounded-b-2xl">
            <div class="flex space-x-3">
              <button
                @click="joinCall(call)"
                :class="[
                  'flex-1 px-4 py-2 rounded-xl font-semibold text-sm transition-all duration-300',
                  call.status === 'live'
              ...
              >
                {{ call.status === 'live' ? 'Join Live' : call.status === 'upcoming' ? 'Start Call' : 'Completed' }}
              </button>
              <button
                @click="toggleDetails(call.id)"
                class="px-4 py-2 border border-gray-300 text-gray-700 hover:border-green-400 hover:text-green-700 rounded-xl font-semibold text-sm transition-all duration-300 flex items-center"
              >
                <span>{{ expandedCall === call.id ? 'Hide Details' : 'View Details' }}</span>
                <svg 
                  class="w-4 h-4 ml-1 transition-transform duration-300" 
                  :class="{ 'rotate-180': expandedCall === call.id }"
                  fill="none" 
                  stroke="currentColor" 
                  viewBox="0 0 24 24"
                >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between">
                      ...
                      <span class="font-medium capitalize">{{ (call.call_type || call.type)?.replace('-', ' ') || 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-gray-600">Record Call:</span>
                      <span class="font-medium">{{ call.record_call === '1' ? 'Yes' : 'No' }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-gray-600">Allow Guests:</span>
                      <span class="font-medium">{{ call.allow_guests === '1' ? 'Yes' : 'No' }}</span>
                    </div>
                  </div>
                </div>

                <!-- Customer Information -->
                <div v-if="call.customer">
                  <h4 class="text-sm font-semibold text-gray-700 mb-2 flex items-center">
                    <svg class="w-4 h-4 mr-2 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div class="bg-white rounded-lg p-3 border border-gray-200">
                    <p class="font-medium text-sm">{{ call.customer.user?.name || 'N/A' }}</p>
                    <p class="text-xs text-gray-500">{{ call.customer.user?.email || 'N/A' }}</p>
                    <p class="text-xs text-gray-500 mt-1">{{ call.customer.address || 'No address' }}</p>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                ...
                <div v-if="call.end_user">
                  ...
                  <div class="bg-white rounded-lg p-3 border border-gray-200">
                    <p class="font-medium text-sm">{{ call.end_user.user?.name || 'N/A' }}</p>
                    <p class="text-xs text-gray-500">{{ call.end_user.user?.email || 'N/A' }}</p>
                    <p class="text-xs text-gray-500 mt-1">Username: {{ call.end_user.username || 'N/A' }}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                      <span class="text-gray-600">Send Reminders:</span>
                      <span class="font-medium">{{ call.send_reminders === '1' ? 'Yes' : 'No' }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-gray-600">Allow File Sharing:</span>
                      <span class="font-medium">{{ call.allow_file_sharing === '1' ? 'Yes' : 'No' }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-gray-600">Allow Anonymous:</span>
                      <span class="font-medium">{{ call.allow_anonymous === '1' ? 'Yes' : 'No' }}</span>
                    </div>
                  </div>
                </div>
              </div>
...
const calls = ref([])
const expandedCall = ref(null) // Track which call is expanded
const error = ref('')
...
// Computed: Filtered & Searched Calls
const filteredCalls = computed(() => {
  ...
  if (searchQuery.value.trim()) {
    ...
    filtered = filtered.filter(call =>
      (call.title?.toLowerCase().includes(query) ?? false) ||
      (call.description?.toLowerCase().includes(query) ?? false) ||
      (call.id?.toString().includes(query) ?? false) ||
      (call.customer?.user?.name?.toLowerCase().includes(query) ?? false) ||
      (call.end_user?.user?.name?.toLowerCase().includes(query) ?? false)
    )
  }
  ...
  if (filters.value.type !== 'all') {
    filtered = filtered.filter(call => 
      (call.call_type === filters.value.type) || (call.type === filters.value.type)
    )
  }
...
// Update stats based on current calls
const updateStats = () => {
  stats.value.totalCalls = calls.value.length
  stats.value.upcomingCalls = calls.value.filter(c => c.status === 'upcoming').length
  stats.value.liveNow = calls.value.filter(c => c.status === 'live').length
  stats.value.completed = calls.value.filter(c => c.status === 'completed').length
}

// Join Call Button
const joinCall = (call) => {
  if (call.status === 'completed') {
    Swal.fire('Ended', 'This call has already ended.', 'info')
    return
  }
  if (call.status !== 'live') {
    Swal.fire('Not Started', 'This call has not started yet.', 'info')
    return
  }

  // Replace with your actual meeting room URL
  const roomUrl = `https://yourdomain.com/meeting/${call.id}`
  ...
  try {
    ...
    if (Array.isArray(data)) {
      calls.value = data.map(call => ({
        ...call,
        // Ensure we have all the fields from your API response
        status: call.status || 'upcoming', // Default to upcoming if no status
    ...
    error.value = err.response?.data?.message || 'Failed to load calls. Please try again.'
