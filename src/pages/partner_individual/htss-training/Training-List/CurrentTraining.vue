<template>
  <div class=" px-4 py-6 w-full mx-auto ">
    ...
    <div class="bg-gradient-to-r from-teal-600 to-emerald-600 rounded-t-2xl text-white px-6 py-5 shadow-lg">
      <h1 class="text-3xl font-bold">🎯Current Training Lists for Customers </h1>
    ...
    <div class="">
      ...
      <div class="my-8 bg-white rounded-2xl p-6 shadow-lg border border-teal-100">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 rounded-xl border border-blue-100">
            ...
            <select v-model="userForm.solutionId" @change="onSolutionChange" required :disabled="loadingSolutions"
          ...
          <div class="bg-gradient-to-br from-purple-50 to-pink-50 p-4 rounded-xl border border-purple-100">
            ...
            <div v-else class="space-y-3">
              <select v-model="userForm.softwareId" :disabled="!userForm.solutionId || availableSoftware.length === 0"
                ...
                <option v-for="soft in availableSoftware" :key="soft.id" :value="soft.id" class="py-2">
                  💻 {{ soft.name }}
                </option>
              </select>
              ...
              <div v-else-if="availableSoftware.length === 0"
                class="text-sm text-purple-600 bg-purple-50 p-3 rounded-lg">
            ...
            <div v-if="userForm.softwareId"
              class="mt-3 text-sm font-medium text-purple-700 bg-purple-100 px-3 py-2 rounded-lg inline-flex items-center">
              <span class="w-2 h-2 bg-purple-500 rounded-full mr-2"></span>
      ...
      <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-teal-100">
        ...
        <div v-else class="overflow-x-auto">
          <div class="overflow-y-auto scrollbar-thin scrollbar-thumb-teal-400 scrollbar-track-gray-100">
            <table class="min-w-full table-fixed border-collapse">

              <thead>
                <tr class="bg-gradient-to-r from-teal-50 to-emerald-50">
                  <th
                    class="p-4 border-r border-teal-200 text-left text-teal-800 font-bold text-sm uppercase tracking-wider">
                    🆔 ID
                  </th>
                  <th
                    class="p-4 border-r border-teal-200 text-left text-teal-800 font-bold text-sm uppercase tracking-wider">
              ...
              <tbody>
                <tr v-for="(training, index) in filteredTrainings" :key="training.id"
                  ...
                  <td class="p-4 border-r border-teal-100">
                    <span :class="`px-3 py-1 rounded-full text-sm font-bold ${training.training_level === 'Beginner' ? 'bg-green-100 text-green-800' :
                  ...
                  <td class="p-4 border-r border-teal-100">
                    <span :class="`px-3 py-2 rounded-full text-xs font-bold flex items-center justify-center w-24 ${training.training_post_status === 'published' ? 'bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-700 border border-blue-200' :
          ...
          <!-- Empty State -->
          <div v-if="filteredTrainings.length === 0 && !loadingTrainings"
            class="text-center py-16 bg-gradient-to-br from-gray-50 to-blue-50">
...
const fetchSolutions = async () => {
  ...
  try {
    const response = await api().get("/solutions?with=softwares", {})
...
const fetchTrainings = async (filters = {}) => {
  ...
  try {
    // let apiUrl = `/training-course?with=industry,software,solution,customer.user&per_page=15`
    let apiUrl = `/training-course?with=customer,solution,software,industry&relation&relation_field=company_id&relation_value=${authStore.companyId}&per_page=15`



    if (filters.solutionId) {
      apiUrl = apiUrl + '&solution_id=' + filters.solutionId

    }

    if (filters.softwareId) {
      apiUrl = apiUrl + '&software_id=' + filters.softwareId
