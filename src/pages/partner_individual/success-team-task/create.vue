...
const taskTypes = [
  {
   value : "Training" ,
   name : "Customer Success Training Team"} ,
   {
 value :  "Engineering Service" ,
 name:    "Customer Success Engineering Team"
},
 {
   value : "Sales Champion",
   name :"Customer Success Sales Team"
  }]
...
const loadDropdowns = async () => {
  try {
    ...
    let customerIds = successTeams.value .map((team)=>team.company?.customers.map((e) => e.id))
      ...
      <form class="p-8 gap-5 grid grid-cols-2" @submit.prevent="submitForm">
        ...
        <div class="form-control">
          <label class="label">
            ...
            <div v-if="!successTeams.length" class="w-full h-10 bg-gray-200 rounded-lg animate-pulse"></div>
          <select
          v-else
          ...
          >
            <option v-for="successTeam in successTeams" :key="successTeam.id" :value="successTeam?.id">
              {{ successTeam?.name }}
            </option>
          </select>
        </div>
        ...
        <div class="form-control">
          <label class="label">
            <span class="label-text font-semibold text-gray-800">
              Task Team<span class="text-red-500">*</span>
            </span>
          </label>
          <select
            v-model="form.type"
          ...
          >
            ...
            <option v-for="u in taskTypes" :key="u" :value="u.value">
              {{ u.name }}
            </option>
          </select>
        </div>
