      ...
      <header class="mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="mt-2 text-lg text-base-content/70">
            Completed All Tasks assigned to {{ authStore.user?.name || 'you' }}
          </p>
        </div>
      </header>
        ...
        <table class="min-w-full text-sm table-zebra">
          <thead class="bg-sky-200 text-gray-600">
            <tr>
              ...
              <th class="px-4 py-3 border-e text-left font-medium">Assign Date</th>
...
const loadAssignments = async () => {
  ...
  try {
    ...
    assignments.value = res.data.data.filter(item => item.status == 'completed').map(item => ({
      id: item.id,
