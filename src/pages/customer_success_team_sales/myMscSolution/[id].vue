...
const getApiVariable = (value) => {
    if (value === 'Structure Analysis Service') {
        return 'A. Structure Analysis Service'
    } else if (value === 'System Dynamics Analysis Service') {
        return 'B. System Dynamics Analysis Service'
    } else if (value === 'Acoustics Analysis Service') {
...
const fetchLeads = async (page = 1) => {
  try {
    ...
    let apiUrl = `/crm/record-child-get/0/Accounts-Deals?per_page=${perPage.value}&page=${page}&filters[c1_315._engineering_segment]=${getApiVariable(route.params.id)}&company_id=${authStore.companyId}`
