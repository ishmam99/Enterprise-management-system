    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
      <div class="bg-gradient-to-r from-teal-700 to-teal-800 text-white px-6 py-4">
        <div class="flex justify-between items-center">
          <h1 class="text-2xl font-semibold">System List</h1>
          <span class="bg-teal-600 text-xs font-medium px-2.5 py-0.5 rounded-full">
            {{ services.length }} systems
          </span>
        </div>
      </div>
      ...
      <div class="p-6">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          >
            ...
            <div class="mb-4">
              <p v-if="!service.editing" class="text-gray-600 text-sm mt-1">{{ service.description || 'No description provided' }}</p>
              <textarea
                v-else
            ...
            <div class="text-xs text-gray-400 mt-2">
              Status: {{ service.status==1 ? 'Active' : 'Inactive' }}<br />
            </div>
          </div>
        </div>
      </div>
...
const deleteService = async (service) => {
  try {
    await api().delete(`/industries/${service.id}`)
