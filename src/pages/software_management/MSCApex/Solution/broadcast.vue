...
// Broadcast solution
const broadcastSolution = async (id) => {
  try {
    ...
    const result = await Swal.fire({
      ...
      html: `
        <div class="text-left">
          <p>Are you sure you want to broadcast this solution to clients?</p>
          ...
          <p class="text-sm text-gray-600 mt-2">This will make the solution visible to potential clients.</p>
        </div>
      `,
      icon: 'question',
      ...
      confirmButtonText: 'Yes, Broadcast Solution!',
      cancelButtonText: 'Cancel'
    })

    if (!result.isConfirmed) return

    // await api().post(`broadcast-solution/${id}`, { status: 4 })

    Swal.fire({
      icon: 'success',
      title: 'Solution Broadcasted!',
      html: `
        <div class="text-center">
          <p>The solution has been broadcasted successfully!</p>
          <div class="mt-3 bg-green-50 p-3 rounded">
            <p><strong>${solution.solution_name}</strong></p>
            <p>by ${solution.freelancer}</p>
            <p class="text-sm text-gray-600 mt-1">Now visible to clients</p>
          </div>
        </div>
      `,
    ...
    // ✅ Remove the broadcasted solution from the list
...
<template>
  <div class="container mx-auto p-4">
    ...
    <div class="flex items-center justify-between bg-teal-700 text-white p-4 mb-4 rounded-lg">
      <h1 class="text-xl font-bold">Solution Broadcast ({{ solutions.length }})</h1>
    </div>
      ...
      <table class="table w-full">
        ...
        <tbody>
          <tr v-for="solution in solutions" :key="solution.id" class="border-b hover:bg-gray-50">
            ...
            <td class="p-3 border-e">
              <button
                @click="broadcastSolution(solution.id)"
