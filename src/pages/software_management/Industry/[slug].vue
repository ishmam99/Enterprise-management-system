    ...
    <div class="w-full mx-auto">
      ...
      <header class="mb-1 text-center">
        <h1 class="text-4xl py-2 md:text-5xl font-bold bg-gradient-to-r from-blue-800 to-slate-800 bg-clip-text text-transparent">
          Industry Details for <span class="text-fuchsia-700">{{ systemData?.name }}</span>
        </h1>
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-2 w-full gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl w-full shadow-lg p-6 lg:col-span-2 border border-slate-200">
          <h2 class="text-xl font-bold text-slate-800 mb-6 border-b border-slate-200 pb-3">Assigned Solutions , Softwares and Registered Customers</h2>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-white p-5 rounded-xl border border-blue-100 shadow-sm transition-all duration-300 hover:shadow-md">
              <div class="flex justify-between items-start mb-3">
                <h3 class="text-sm font-semibold text-blue-800 uppercase tracking-wide">Assigned Solutions</h3>
              ...
              <p class="text-3xl font-bold text-blue-900 mb-2">{{ formatNumber(systemData.solutions?.length || 0) }}</p>
            </div>
            ...
            <div class="bg-gradient-to-br from-red-50 to-white p-5 rounded-xl border border-red-100 shadow-sm transition-all duration-300 hover:shadow-md">
              <div class="flex justify-between items-start mb-3">
                <h3 class="text-sm font-semibold text-red-800 uppercase tracking-wide">Registered Customers</h3>
              ...
              <p class="text-3xl font-bold text-red-900 mb-2">{{ formatNumber(systemData.customers?.length || 0) }}</p>
            </div>
              <div class="bg-gradient-to-br from-yellow-50 to-white p-5 rounded-xl border border-yellow-100 shadow-sm transition-all duration-300 hover:shadow-md">
              <div class="flex justify-between items-start mb-3">
                <h3 class="text-sm font-semibold text-yellow-800 uppercase tracking-wide">Associated Training Courses</h3>
                <div class="w-10 h-10 rounded-lg bg-yellow-600 flex items-center justify-center shadow-sm">
              <i class="ri-presentation-line text-white text-xl"></i>
                </div>
              </div>
              <p class="text-3xl font-bold text-yellow-900 mb-2">{{ formatNumber(systemData?.trainings?.length || 0) }}</p>
            </div>
        ...
        <div class="px-3 py-2 bg-amber-400 rounded-xl shadow-lg space-y-2">
          <h1 class="text-2xl text-white font-semibold underline">Solutions Assigned :</h1>
          <div class="grid grid-cols-3 gap-3" >
          <div v-for="solution in systemData?.solutions" :key="solution.id" class="rounded-2xl bg-gray-500 flex justify-center items-center p-2 text-center text-white">
              <p class="text-nowrap">{{ solution.name }}</p>
          </div>
        </div>
        </div>
        <div class="px-3 py-2 col-span-2 bg-green-400 rounded-xl shadow-lg space-y-2">
          <h1 class="text-2xl text-white font-semibold underline">Customers Registered :</h1>
      ...
      <table class="min-w-full text-sm text-gray-700 table-zebra">
        ...
        <div class="px-3 py-2 col-span-2 bg-blue-400 rounded-xl shadow-lg space-y-2">
          <h1 class="text-2xl text-white font-semibold underline">Associated Training Courses :</h1>
            <div class="overflow-x-auto bg-white rounded-2xl shadow-lg border border-gray-100">
      <table class="min-w-full text-sm text-gray-700 table-zebra">
        <thead class="bg-blue-600 text-white text-left">
          <tr>
            <th class="py-3 border-e px-4">#</th>
            <th class="py-3 border-e px-4">Name</th>
            <th class="py-3 border-e px-4">ID</th>
            <th class="py-3 border-e px-4">Solution</th>
            <th class="py-3 border-e px-4">Software</th>
            <th class="py-3 border-e px-4">Level</th>
            <th class="py-3 border-e px-4">Type</th>
            <th class="py-3 border-e px-4">Duration</th>
            <th class="py-3 border-e px-4 text-right">Status</th>
          </tr>
        </thead>
        <tbody>
          <tr
            v-for="(training, index) in systemData?.trainings"
            :key="training.id"
            class="border-b border-gray-100 hover:bg-gray-50 transition"
          >
            <td class="py-3 border-e px-4">{{ index + 1 }}</td>
            <td class="py-3 border-e px-4 font-semibold">{{ training.title }}</td>
            <td class="py-3 border-e px-4 text-nowrap">{{training.course_id }}</td>
            <td class="py-3 border-e px-4 capitalize">{{ training.solution.name }}</td>
            <td class="py-3 border-e px-4 capitalize">{{ training.software.name }}</td>
            <td class="py-3 border-e px-4 capitalize">{{ training.training_level }}</td>
            <td class="py-3 border-e px-4 capitalize">{{ training.training_type }}</td>
            <td class="py-3 border-e px-4 capitalize">{{ training.duration }}</td>
            <td class="py-3 border-e px-4">
              <span
                :class="[
                  'px-3 py-1 rounded-full text-xs text-nowrap font-semibold',
                 training.status == 1
                    ? 'bg-emerald-100 text-emerald-700'
                    : 'bg-gray-200 text-gray-600',
                ]"
              >
                {{ training.status == 1 ? 'Active' : 'In-Active' }}
              </span>
            </td>
            <td class="py-3 px-4 text-right text-gray-500">{{ training.lastActive }}</td>
          </tr>
          <tr v-if="systemData?.trainings === 0">
            <td colspan="6" class="text-center py-6 text-gray-400">No training courses found</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const getPlanningStats = async () => {
  try {
    ...
    const { data } = await api().get(`industries/${systemId.value}?softwares=true&solutions=true&customers=true&trainings=true`)
