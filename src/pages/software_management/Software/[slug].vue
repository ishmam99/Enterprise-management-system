    ...
    <div class="w-full mx-auto">
      ...
      <header class="mb-1 text-center">
        <h1 class="text-4xl py-2 md:text-5xl font-bold bg-gradient-to-r from-blue-800 to-slate-800 bg-clip-text text-transparent">
          Software Details for <span class="text-red-700">{{ systemData?.name }}</span>
        </h1>
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-2 w-full gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl w-full shadow-lg p-6 lg:col-span-2 border border-slate-200">
          <h2 class="text-xl font-bold text-slate-800 mb-6 border-b border-slate-200 pb-3">Assigned Industries , Solutions and Registered Customers</h2>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <!-- Total Service Taken -->
            <div class="bg-gradient-to-br from-pink-50 to-white p-5 rounded-xl border border-pink-100 shadow-sm transition-all duration-300 hover:shadow-md">
              <div class="flex justify-between items-start mb-3">
                <h3 class="text-sm font-semibold text-pink-800 uppercase tracking-wide">Assigned Solutions</h3>
                <div class="w-10 h-10 rounded-lg bg-pink-600 flex items-center justify-center shadow-sm">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                  </svg>
                </div>
              </div>
              <p class="text-3xl font-bold text-pink-900 mb-2">{{ formatNumber(systemData?.solutions?.length || 0) }}</p>
            </div>
            <div class="bg-gradient-to-br from-amber-50 to-white p-5 rounded-xl border border-amber-100 shadow-sm transition-all duration-300 hover:shadow-md">
              <div class="flex justify-between items-start mb-3">
                <h3 class="text-sm font-semibold text-amber-800 uppercase tracking-wide">Registered Customers</h3>
                <div class="w-10 h-10 rounded-lg bg-amber-600 flex items-center justify-center shadow-sm">
              <i class="ri-group-line text-white text-xl"></i>
                </div>
              </div>
              <p class="text-3xl font-bold text-amber-900 mb-2">{{ formatNumber(systemData?.customers?.length || 0) }}</p>
            </div>
        ...
        <div class="px-3 py-2 bg-sky-400 rounded-xl shadow-lg space-y-2">
          ...
          <div v-for="software in systemData?.solutions" :key="software.id" class="rounded-2xl bg-pink-500 flex justify-center items-center p-2 text-white">
              <p>{{ software.name }}</p>
          </div>
        </div>
        </div>
        <div class="px-3 py-2 bg-amber-400 rounded-xl shadow-lg space-y-2">
          ...
          <div v-for="industry in systemData?.industries" :key="industry.id" class="rounded-2xl bg-gray-500 flex justify-center items-center p-2 text-center text-white">
              <p class="text-wrap">{{ industry.name }}</p>
          </div>
        </div>
        </div>
...
const getPlanningStats = async () => {
  try {
    ...
    const { data } = await api().get(`softwares/${systemId.value}?solutions=true&industries=true&customers=true&trainings=true`)
