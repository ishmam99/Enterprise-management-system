      ...
      <header class="mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="mt-2 text-lg text-base-content/70">
            Tasks assigned to {{ authStore.user?.name || 'you' }}
          </p>
        </div>
      </header>
      ...
      <div class="mb-8">
        <label class="input input-bordered flex items-center gap-2 w-full">
          ...
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Search by solution, software, description..."
            class="grow"
          />
        </label>
      </div>
        ...
        <table class="min-w-full text-sm table-zebra">
          <thead class="bg-sky-200 text-gray-600">
            <tr>
              ...
              <th class="px-4 py-3 border-e text-left font-medium">Status</th>
              <th class="px-4 py-3 border-e text-left font-medium">Created At</th>
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-3 border-e font-medium">{{ assignment.solution?.name || '—' }}</td>
              <td class="px-4 py-3 border-e">{{ assignment.software?.name || '—' }}</td>
              <td class="px-4 py-3 border-e">{{ assignment.assignedTo?.name || '—' }}</td>
              <td class="px-4 py-3 border-e max-w-xs break-words whitespace-normal">
              ...
              <td class="px-4 py-3 border-e">
                <span class="badge" :class="statusColor(assignment.status)">
                  {{ assignment.status }}
                </span>
              </td>

              <td class="px-4 py-3 border-e">{{ formatDate(assignment.createdAt) }}</td>

              <!-- Outputs -->
              <td class="px-4 py-3 border-e">
                <div class="flex flex-col gap-2">
                  <div
                    v-for="output in assignment.outputs"
                    :key="output.id"
                    class="flex items-center justify-between gap-2 bg-gray-50 rounded-lg p-2 shadow-sm"
                  >
                    <div class="flex flex-col">
                      <span class="text-sm text-gray-800">{{ output.output }}</span>
                      <span
                    ...
                    <div class="flex gap-2">
                      ...
                      >
                        ...
                        <span>{{ output.status == 0 ? 'Post' : 'Un-Post' }}</span>
                      </button>
                      <button
                  ...
                  <div v-if="!assignment.outputs.length" class="text-gray-500 text-xs">No Output</div>
                </div>
              </td>
              <!-- Add Action Button -->
              <td class="px-4 py-3 border-e text-center">
                <button
                  @click="openAddModal(assignment.id)"
...
const deleteOutput = async (outputId) => {
  ...
  if (res.isConfirmed) {
    await api().delete(`/success-team-task/${outputId}`)
