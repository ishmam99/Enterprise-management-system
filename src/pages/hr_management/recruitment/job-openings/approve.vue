            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="job in jobOpenings" :key="job.id" class="hover:bg-gray-50 transition-colors">
                ...
                <div class="flex items-center justify-center gap-4">
                    ...
                    class="font-medium text-sm text-green-600 hover:text-green-800"
                    >
                    Approve
                    </button>

                    <span v-else class="text-gray-500 text-sm">
                    {{ job.status === '1' ? 'Approved' : statusText(job.status) }}
                    </span>
                </div>
                </td>
              </tr>
...
const fetchData = async () => {
  try {
    ...
    Swal.fire({
      icon: 'error',
      title: 'Oops!',
      text: 'Failed to load job openings. Please try again.'
    })
  } finally {
    loading.value = false
  }
...
const togglePublish = async (id, currentStatus) => {
  const action = currentStatus === '0' ? 'unpublish' : 'publish'
  ...
  try {
    ...
    // Toggle between Draft (1) and Published (0)
    const job = jobOpenings.value.find(j => j.id === id)
    if (job) {
      job.status = job.status === '0' ? '1' : '0'
    }

    alert(`Job ${job.status === '0' ? 'published' : 'unpublished (set to draft)'}.`)
  } catch (error) {
