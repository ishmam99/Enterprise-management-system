        ...
        <table class="min-w-full border-collapse border border-gray-300">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border border-gray-300" style="min-width: 200px; max-width: 250px;">
                <div class="flex items-center gap-1">
                  <i class="ri-briefcase-line"></i>
                  Product Name
                </div>
              </th>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border border-gray-300" style="min-width: 180px; max-width: 220px;">
                <div class="flex items-center gap-1">
              ...
              <template v-for="day in days" :key="day.date">
                ...
                <template v-else>
                  ...
                  >
                    ...
                    <!-- PRODUCT NAME -->
                    <td class="px-4 py-3 border border-gray-300" style="min-width: 200px; max-width: 250px;">
                      <div v-if="timeEntry.product?.name" class="flex flex-col">
                        <span class="font-medium text-gray-900 break-words">{{ timeEntry.product.name }}</span>
                      </div>
                      <span v-else class="text-gray-500">—</span>
                    </td>

                    <!-- MODULE -->
                    <td class="px-4 py-3 border border-gray-300" style="min-width: 180px; max-width: 220px;">
                      <div v-if="timeEntry.module?.name" class="flex flex-col">
                        <span class="font-medium text-gray-900 break-words">{{ timeEntry.module.name }}</span>
                        <span v-if="timeEntry.notes" class="text-xs text-gray-500 truncate">
                          {{ timeEntry.notes }}
                        </span>
                      </div>
                      <span v-else class="text-gray-500">—</span>
                    ...
                    <td class="px-4 py-3 whitespace-nowrap border border-gray-300">
                      <div v-if="editingId !== timeEntry.id">
                        <div v-if="timeEntry.major_task" class="flex flex-col">
                          <span class="font-medium text-gray-900">{{ timeEntry.major_task }}</span>
                        </div>
                        <span v-else class="text-gray-500">—</span>
                      </div>
                      <input
                        v-else
                        v-model="editForm.major_task"
                        type="text"
                        class="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="Major Task Name"
                      />
                    </td>

                    <!-- MAJOR SUB TASK NAME -->
                    <td class="px-4 py-3 border border-gray-300" style="min-width: 180px; max-width: 220px;">
                      <span v-if="editingId !== timeEntry.id" class="font-medium text-gray-900 break-words">{{ timeEntry.major_sub_task || '—' }}</span>
                      <input
                        v-else
                        v-model="editForm.major_sub_task"
                        type="text"
                        class="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="Major Sub Task Name"
                      />
                    </td>
                  ...
                  >
                    ...
                    <td class="px-4 py-3 text-gray-500 border border-gray-300" style="min-width: 200px; max-width: 250px;">—</td>
                    <td class="px-4 py-3 text-gray-500 border border-gray-300" style="min-width: 180px; max-width: 220px;">—</td>
...
const startEdit = (timeEntry) => {
  ...
  editForm.value = {
    major_task: timeEntry.major_task || '',
    major_sub_task: timeEntry.major_sub_task || '',
...
const saveEdit = async (timeEntry, date) => {
  try {
    ...
    updatedTimes[timeIndex] = {
      ...updatedTimes[timeIndex],
      major_task: editForm.value.major_task || updatedTimes[timeIndex].major_task,
      major_sub_task: editForm.value.major_sub_task || updatedTimes[timeIndex].major_sub_task,
    ...
    const { data } = await api().post(`attendance/${attendance.id}`, {
      ...
      times: updatedTimes.map(t => ({
        id: t.id,
        major_task: t.major_task,
        major_sub_task: t.major_sub_task,
        description: t.description,
        output: t.output,
        total_hours: t.total_hours,
        total_minute: t.total_minute,
        module_id: t.module?.id || null,
