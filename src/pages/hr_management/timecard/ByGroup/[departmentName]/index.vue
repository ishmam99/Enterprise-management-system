<template>
  <div class="p-6 min-h-screen w-full bg-gray-50">
    ...
    <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
      <p class="text-blue-700 font-medium">📊 Department Timecard View</p>
      <p class="text-blue-600 text-sm">🏢 Department: <strong>{{ departmentName || 'Loading...' }}</strong></p>
      <p class="text-blue-600 text-sm">⏳ Loading: <strong>{{ loading ? 'Yes' : 'No' }}</strong></p>
      <p class="text-blue-600 text-sm">📋 Records: <strong>{{ timecards.length }}</strong></p>
    </div>
...
// Get department name from route - could be in params or path
const deptName = route.params.departmentName || route.query.dept || ''
const departmentName = ref(typeof deptName === 'string' ? decodeURIComponent(deptName) : deptName)
const { hrmApi } = useHRM()
...
const employees = ref([])
const meta = ref(null)
const loading = ref(true) // Set to true initially
...
const fetchTimeCards = async () => {
  try {
    ...
    console.log('Fetched timecards:', timecards.value)
...
watch(() => route.params.departmentName, (newDeptName) => {
  if (newDeptName) {
    ...
    console.log('Department changed to:', departmentName.value)
    currentPage.value = 1
    fetchTimeCards()
  }
})
