        ...
        <table class="w-full border-collapse text-sm">
          ...
          <thead>
            <tr class="text-center font-bold">
              ...
              <th colspan="9" class="border p-3 bg-green-50 text-green-700">
                <div class="flex items-center justify-center gap-2">
                  <!-- <i class="ri-task-line"></i> -->
                  Task / Work Details
                </div>
              </th>
              <th colspan="3" class="border p-3 bg-purple-50 text-purple-700">
                <div class="flex items-center justify-center gap-2">
                  <!-- <i class="ri-check-double-line"></i> -->
                  Approval Status
                </div>
            ...
            <tr>
              ...
              <th class="border p-3 bg-green-50 text-left">Account</th>
              <th class="border p-3 bg-green-50 text-left">Task Type</th>
              <th class="border p-3 bg-green-50 text-left">Task Name</th>
          ...
          <tbody>
            <!-- LOADING STATE -->
            <tr v-if="loading">
              <td colspan="15" class="px-4 py-8 border">
              ...
              <template v-for="day in days" :key="day.date">
                <template v-for="(timeEntry, timeIndex) in attendanceByDate[day.date]?.times || [{}]"
                  ...
                  <tr :class="[
                      ...
                      <div v-if="attendanceByDate[day.date]?.attendance_logs?.length" class="space-y-2">
                        <div class="space-y-1">
                          <div class="flex items-center gap-1 text-green-600">
                            ...
                            <span class="font-medium">First Login:</span>
                            <span>{{ getFirstLoginTime(day.date) }}</span>
                          </div>
                          <div class="flex items-center gap-1 text-red-600">
                            <i class="ri-logout-box-line"></i>
                            <span class="font-medium">Last Logout:</span>
                            <span>{{ getLastLogoutTime(day.date) }}</span>
                          </div>
                        </div>
                        <div class="text-xs text-gray-500">
                          {{ attendanceByDate[day.date]?.attendance_logs?.length || 0 }} session(s)
                        </div>
                      </div>
                    ...
                    <!-- TASK NAME -->
                    <td class="border p-2">
                      <span class="font-medium text-gray-900">{{ timeEntry.task_name || '—' }}</span>
                    </td>
...
<td class="border p-2 text-center">
  <div v-if="timeEntry.total_hours || timeEntry.total_hours === 0" class="bg-gray-50 p-2 rounded-lg">
    <span class="font-bold text-gray-900">{{ timeEntry.total_hours || 0 }}</span>
    <span class="text-xs text-gray-500 ml-1">h</span>
  </div>
  <div v-else class="py-4">
    <span class="text-gray-400">—</span>
  </div>
...
<!-- MINUTES -->
<td class="border p-2 text-center">
  <div v-if="timeEntry.total_minute || timeEntry.total_minute === 0" class="bg-gray-50 p-2 rounded-lg">
    <span class="font-bold text-gray-900">{{ (timeEntry.total_minute || 0).toString().padStart(2, '0') }}</span>
...
const getTotalLoginTime = (date) => {
  ...
  if (!attendance?.attendance_logs?.length) return { hours: 0, minutes: 0 }

  let totalMinutes = 0
  attendance.attendance_logs.forEach(log => {
    if (log.login_time && log.logout_time) {
      const login = moment(log.login_time, 'HH:mm:ss')
      const logout = moment(log.logout_time, 'HH:mm:ss')
...
const loadAttendanceForWeek = async () => {
  ...
  for (const day of days.value) {
    try {
      const res = await hrmApi.get(`/timecard-info-of-user/${userId.value}/${day.date}/${day.date}`)
