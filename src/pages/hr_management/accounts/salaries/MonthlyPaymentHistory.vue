...
const downloadPDF = () => {
  ...
  doc.text(`Monthly Payment History - ${currentDate.value.format('MMMM YYYY')}`, 105, 10, { align: 'center' })
  ...
  // Summary
  doc.setFontSize(10)
  doc.text(`Total Employees: ${data.length} | Paid: ${paymentStats.value.paid} | Pending: ${paymentStats.value.pending}`, 105, 25, { align: 'center' })

  // Table
  autoTable(doc, {
    head: [
      ['#', 'Name', 'Employee ID', 'Monthly Salary', 'Working Days', 'Net Payable', 'Payment Date', 'Status']
    ],
    ...
    columnStyles: {
      3: { halign: 'right' },  // Monthly Salary
      5: { halign: 'right' }    // Net Payable
    },
    styles: {
      fontSize: 8,
      cellPadding: 3,
  ...
  doc.save(`payment-history-${currentDate.value.format('YYYY-MM')}.pdf`)
}
...
<template>
  <SectionMain class="mx-10">
    ...
    :title="`Monthly Payment History - ${currentDate.format('MMMM YYYY')}`"
    main
  >
        ...
        <table class="w-full border-2 border-gray-200 shadow-xl">
          ...
          <tbody>
            ...
            <tr v-else-if="displayedSalaries.length === 0">
              <td colspan="8" class="p-4 text-center">No records found</td>
            </tr>
            <tr 
