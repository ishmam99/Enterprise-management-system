...
const getIndependentUsers = async () => {
  ...
  try {
    // First, let's get all users from the users endpoint
    // Adjust the endpoint based on your actual API structure
    const response = await api().get(
      `users?per_page=${perPage}&page=${currentPage.value}&search=${encodeURIComponent(searchQuery.value)}`
    )

    // Filter users to find those not associated with any customer
    // This is a client-side filter since we're using the same API
    const allUsers = response.data.data.data || []
    
    // Now we need to check which users don't have customer associations
    // We'll fetch customers and cross-reference
    const customersResponse = await api().get('customers?with=user')
    const customers = customersResponse.data.data.data || []
    
    // Get user IDs that are associated with customers
    const customerUserIds = customers.map(customer => customer.user_id).filter(id => id)
    
    // Filter users to only show those not in customerUserIds
    users.value = allUsers.filter(user => !customerUserIds.includes(user.id))
    
    // If role filter is applied, filter by role as well
    ...
    // Fallback: Try to get users from a different endpoint or show empty state
    try {
      // Alternative approach: get all customers and extract users that don't have customer records
      const customersResponse = await api().get('customers?with=user')
      const customers = customersResponse.data.data.data || []
      const customerUserIds = customers.map(customer => customer.user_id).filter(id => id)
      
      // For demo purposes, we'll create some mock independent users
