...
const fields = ref([])
const dealEngineeringSegmentOptions = ref([])
const dealSoftwareName = ref([])
...
const selectedEngineeringSegmnent = ref('')
...
const hasActiveFilters = computed(() => {
    return selectedEngineeringSegmnent.value ||
...
const selectFilter = (type, value) => {
    switch (type) {
        case 'engineeringSegment':
            selectedEngineeringSegmnent.value = value
            activeFilters.engineeringSegment = value
            break
        case 'softwareName':
            selectedSoftwareName.value = value
            activeFilters.softwareName = value
            break
        case 'stage':
...
const clearFilter = (type) => {
    switch (type) {
        case 'engineeringSegment':
            selectedEngineeringSegmnent.value = ''
            activeFilters.engineeringSegment = ''
            break
        case 'softwareName':
            selectedSoftwareName.value = ''
            activeFilters.softwareName = ''
            break
        case 'stage':
            selectedStage.value = ''
            activeFilters.stage = ''
...
const fetchFields = async () => {
    ...
    fields.value = data.data.sort((a, b) => {
...
const getDropdownList = async () => {
    try {
        dealEngineeringSegmentOptions.value =
            fields.value?.find((e) => e?.id == 471)?.options || []
        dealSoftwareName.value = fields.value?.find((e) => e?.id == 481)?.options || []
        dealStage.value = fields.value?.find((e) => e?.id == 53)?.options || []
        ...
        console.log('Engagement options:', dealEngineeringSegmentOptions.value);
...
const fetchLeads = async (page = 1) => {
    ...
    try {
        ...
        // Build base API URL
        let apiUrl = `/crm/record-child-get/0/Accounts-Deals?page=${page}&per_page=${perPage.value}&company_id=${authStore.companyId}&filters[deal_status]=${getApiVariable(route.params.id)}`
        ...
        if (selectedFilter.value === 'HTSS') {
            ...
            htssOptions.forEach(option => {
                apiUrl += `&filters[deal_originated_by]=${option}`
            })
        } else if (selectedFilter.value === 'MSC') {
            apiUrl += '&filters[deal_originated_by]=1a. MSC-Hexagon&filters[deal_originated_by]=1b. MSC-Cadence'
        }
        if (selectedEngineeringSegmnent.value) {
            apiUrl += `&filters[c1_315._engineering_segment]=${selectedEngineeringSegmnent.value}`
        }

        if (selectedSoftwareName.value) {
            apiUrl += `&filters[d0_001._software_name]=${selectedSoftwareName.value}`
        }

        if (selectedStage.value) {
            apiUrl += `&filters[stage]=${selectedStage.value}`
        }
    ...
    <div class="bg-gray-50 min-h-screen py-8">
        <div class="container mx-auto">
            ...
            <div class="bg-white rounded-2xl shadow-lg px-6 py-2">
                <p v-if="selectedFilter == 'HTSS'" class="text-2xl font-bold text-blue-600">HTSS Champion Contacts{{ }}
                </p>
                <div class="flex justify-between">
                    <h2 class="text-2xl font-bold text-gray-800 pb-2"> {{
                        getHeading(route.params.id) }} Deals - {{ authStore.companyName }}
                    </h2>
                ...
                <div class="flex mb-6 gap-4 flex-wrap">
                    ...
                    <div v-if="isLoadingDropdowns == false" class="relative flex">
                        ...
                        ">
                            ...
                            <!-- Deal Engineering Segment Dropdown -->
                            <div class="relative w-80">
                                <div class="mb-2">
                                    <div class="flex items-center justify-between">
                                        <h3 class="text-sm font-semibold text-gray-700 mb-1">Engineering Segment</h3>
                                        <button v-if="selectedEngineeringSegmnent"
                                            @click="clearFilter('engineeringSegment')"
                                            class="text-xs text-red-500 hover:text-red-700">
                                            Clear
                                        </button>
                                    </div>
                                    <button @click="toggleDropdown('engineeringSegment')" data-filter-button :class="[
                                        'flex items-center justify-between w-full px-4 py-2 rounded-lg font-medium transition-all duration-200 border',
                                        selectedEngineeringSegmnent
                                            ? 'bg-gradient-to-r from-purple-500 to-purple7w-720 text-white shadow-md border-purple-600'
                                            : 'bg-white text-gray-700 hover:bg-gray-50 border-gray-300 hover:border-gray-400'
                                    ]">
                                        <div class="flex items-center gap-2">
                                            <span class="font-medium truncate">
                                                {{ selectedEngineeringSegmnent || 'Select Engineering Segment' }}
                                            </span>
                                        </div>
                                        <svg class="w-4 h-4 transition-transform duration-300"
                                            :class="{ 'rotate-180': activeDropdown === 'engineeringSegment' }"
                                ...
                                <transition enter-active-class="transition duration-150 ease-out"
                                    ...
                                    leave-from-class="transform scale-100 opacity-100"
                                    leave-to-class="transform scale-95 opacity-0">
                                    <div v-show="activeDropdown === 'engineeringSegment'"
                                        ...
                                        @click.stop data-dropdown>
                                        <button v-for="option in dealEngineeringSegmentOptions" :key="option"
                                            @click="selectFilter('engineeringSegment', option)" :class="[
                                                'flex items-center justify-between w-full px-4 py-2 text-left transition-all duration-200 hover:bg-purple-50',
                                                selectedEngineeringSegmnent === option
                                                    ? 'bg-purple-50 text-purple-700 font-semibold'
                                                    : 'text-gray-700 hover:text-purple-600'
                                            ]">
                                            <span class="truncate">{{ option }}</span>
                                            <svg v-if="selectedEngineeringSegmnent === option"
                            ...
                            <!-- Deal Software Name Dropdown -->
                            <div class="relative w-80">
                                <div class="mb-2">
                                    <div class="flex items-center justify-between">
                                        <h3 class="text-sm font-semibold text-gray-700 mb-1">Software Name</h3>
                                        <button v-if="selectedSoftwareName" @click="clearFilter('softwareName')"
                                            class="text-xs text-red-500 hover:text-red-700">
                                            Clear
                                        </button>
                                    </div>
                                    <button @click="toggleDropdown('softwareName')" data-filter-button :class="[
                                        'flex items-center justify-between w-full px-4 py-2 rounded-lg font-medium transition-all duration-200 border',
                                        ...
                                        <div class="flex items-center gap-2">
                                            <span class="font-medium truncate">
                                                {{ selectedSoftwareName || 'Select Software Name' }}
                                            </span>
                                        </div>
                                        <svg class="w-4 h-4 transition-transform duration-300"
                                            :class="{ 'rotate-180': activeDropdown === 'softwareName' }" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 9l-7 7-7-7" />
                                        </svg>
                                ...
                                <transition enter-active-class="transition duration-150 ease-out"
                                    ...
                                    leave-from-class="transform scale-100 opacity-100"
                                    leave-to-class="transform scale-95 opacity-0">
                                    <div v-show="activeDropdown === 'softwareName'"
                                        class="absolute z-20 w-full mt-1 bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden max-h-64 overflow-y-auto"
                                        @click.stop data-dropdown>
                                        <button v-for="option in dealSoftwareName" :key="option"
                                            @click="selectFilter('softwareName', option)" :class="[
                                                'flex items-center justify-between w-full px-4 py-2 text-left transition-all duration-200 hover:bg-green-50',
                                                selectedSoftwareName === option
                                                    ? 'bg-green-50 text-green-700 font-semibold'
                                                    : 'text-gray-700 hover:text-green-600'
                                            ]">
                                            <span class="truncate">{{ option }}</span>
                                            <svg v-if="selectedSoftwareName === option" class="w-4 h-4 text-green-600"
                                                fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                                                    d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                                                    clip-rule="evenodd" />
                            ...
                            <div class="relative w-80">
                                <div class="mb-2">
                                    <div class="flex items-center justify-between">
                                        <h3 class="text-sm font-semibold text-gray-700 mb-1">Deal Stage</h3>
                                        <button v-if="selectedStage" @click="clearFilter('stage')"
                                            class="text-xs text-red-500 hover:text-red-700">
                                            Clear
                                        </button>
                                    </div>
                                    <button @click="toggleDropdown('stage')" data-filter-button :class="[
                                        'flex items-center justify-between w-full px-4 py-2 rounded-lg font-medium transition-all duration-200 border',
                                        selectedStage
                                        ...
                                        <div class="flex items-center gap-2">
                                            <span class="font-medium truncate">
                                                {{ selectedStage || 'Select Deal Stage' }}
                                            </span>
                                        </div>
                                        <svg class="w-4 h-4 transition-transform duration-300"
                                            :class="{ 'rotate-180': activeDropdown === 'stage' }" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 9l-7 7-7-7" />
                                        </svg>
                                ...
                                <transition enter-active-class="transition duration-150 ease-out"
                                    ...
                                    leave-from-class="transform scale-100 opacity-100"
                                    leave-to-class="transform scale-95 opacity-0">
                                    <div v-show="activeDropdown === 'stage'"
                                        class="absolute z-20 w-full mt-1 bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden max-h-64 overflow-y-auto"
                                        @click.stop data-dropdown>
                                        <button v-for="option in dealStage" :key="option"
                                            @click="selectFilter('stage', option)" :class="[
                                            ...
                                            <span class="truncate">{{ option }}</span>
                                            <svg v-if="selectedStage === option" class="w-4 h-4 text-orange-600"
                                                fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                                                    d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                                                    clip-rule="evenodd" />
