<template>
  <div class="px-4 bg-gradient-to-br from-slate-50 to-slate-100 ">
    <Breadcrumb />
    <div class="backdrop-blur-xl bg-white/70 rounded-2xl shadow-2xl p-5 border border-white/20">
        ...
        <div class="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 class="text-2xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">
              Solution Usability Graph
            </h1>
            <p class="text-xs text-slate-600/70 mt-1 flex items-center gap-2">
              <span class="inline-block w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
              Visual representation of solution usability metrics
            </p>
          </div>
      ...
      <div v-if="loading" class="text-center py-16">
        <div class="relative w-12 h-12 mx-auto">
          ...
          <div class="absolute inset-0 rounded-full border-3 border-emerald-500 border-t-transparent animate-spin">
          </div>
        </div>
      ...
      <div v-else class="w-full ">
        <div class="relative flex items-center justify-center">
          <canvas ref="chartRef" :key="canvasKey" class="w-full" style="height: 750px !important;"></canvas>
        </div>

        <!-- Glass morphism insights summary -->
        <div class="mt-4 pt-3 border-t border-white/30" v-if="totalSolutions > 0">
          <div class="flex flex-wrap gap-4 justify-center text-xs">
            <div class="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/50 backdrop-blur-sm">
              ...
              <span class="px-2 py-0.5 rounded-full font-bold text-white text-xs" :class="getScoreClass(avgUsability)">
                {{ avgUsability }}%
              </span>
            </div>
            <div class="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/50 backdrop-blur-sm">
              <span class="font-semibold text-slate-700">Solutions:</span>
...
const fetchData = async () => {
  ...
  try {
    const res = await api().get(
      '/customer-solution?company_id=' + authStore.companyId
    )
    ...
    raw.forEach(item => {
      const name = item?.solution_name || 'Unknown'
    ...
    totalSolutions.value = totalCount
