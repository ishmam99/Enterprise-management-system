      ...
      <div class="container">
        ...
        <div class="mt-6">
          ...
          <div class="text-center mb-4">
            ...
            <p class="text-lg text-gray-600">Available training courses for {{   selectedFilterItem?.name }}</p>
          </div>
...
const selectSoftware = async (software) => {
  console.log(softwareList.value)
  let softwareName =  software.charAt(0).toUpperCase()+software.slice(1)
  let setsoftware = softwareList.value.find(s => s.name == softwareName);
  console.log(setsoftware, softwareName ,softwareList.value.filter(s => s.name == softwareName) ,softwareList.value)
  selectedFilterItem.value = setsoftware
  
}

const selectSolution = async (solution) => {
  // Ensure first letter is uppercase and append ' Analysis'
  if (typeof solution === 'string' && solution.length > 0) {
    selectedFilterItem.value = solution.charAt(0).toUpperCase() + solution.slice(1) + ' Analysis'
  } else {
...
watch(() => route.query, () => {
  selectSoftware(route.params.name)
});
// == LIFECYCLE ==
onMounted(async() => {
  await loadAllData();
