...
const fetchDeals = async (page = 1) => {
  try {
    ...
    const { data } = await api().get(
      ...
      route.params.type == "Negotiation"
        ? `/crm/modules/5/records?field=stage&value=Negotiation/Review&page=${page}&per_page=${perPage.value}`
        : `/crm/modules/5/records?field=stage&value=${encodedType}&page=${page}&per_page=${perPage.value}`
    )
