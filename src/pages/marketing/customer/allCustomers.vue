...
const fetchLeads = async (page = 1) => {
  try {
    ...
    const { data } = await api().get(`/crm/modules/2/records?page=${page}&per_page=${perPage.value}&field=account_type_new&value=customer `)
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
          <div class="flex items-center gap-1">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><path d="M11 14.0619V20H13V14.0619C16.9463 14.554 20 17.9204 20 22H4C4 17.9204 7.05369 14.554 11 14.0619ZM12 13C8.685 13 6 10.315 6 7C6 3.685 8.685 1 12 1C15.315 1 18 3.685 18 7C18 10.315 15.315 13 12 13Z"></path></svg>
        ...
        <div v-else class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 table-zebra">
            <thead class="bg-gradient-to-r from-emerald-50 to-teal-50">
              <tr>
                <th class="px-6 py-2 border-x text-left font-bold text-emerald-700 uppercase tracking-wider">
                  #
                </th>
            ...
            <tbody class="bg-white divide-y divide-gray-100">
              ...
              <tr v-else v-for="(lead, index) in leads" :key="lead.id"
                ...
                <td v-for="field in fields" :key="field.id"
                  ...
                  <router-link :to="{
                    name: 'sales_management-customer-customerDetails-id',
                    params: { id: lead.id },
        ...
        <div class="flex justify-between items-center mt-6">
          <p class="text-sm text-gray-600">
            Showing page <span class="font-semibold">{{ currentPage }}</span> of <span class="font-semibold">{{
              totalPages }} || Total Customers {{ totalAccounts }}</span>
          </p>
