  ...
  <div class="flex h-screen bg-gray-50 pb-10 mt-2">
    <aside
      ...
      <button
        ...
        <span class="font-semibold text-sm">All Customers</span>
      </button>
      ...
      <div class="bg-white shadow-sm px-8 py-5 border-b border-gray-200">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex gap-2 flex-row-reverse">
            ...
            >
              <button @click="deleteRecord()">Delete Customer</button>
            </div> -->
            <!-- <div
             v-if="lead?.values.find((e) => e.field?.name == 'account_type_new')?.value == null || lead?.values.find((e) => e.field?.name == 'account_type_new')?.value == 'Not Customer'"
            ...
            >
              <button @click="convertToCustomer()">Convert to Customer</button>
            </div>
             <div
      ...
      <main class="px-8 pb-10 flex-1 space-y-6">
        ...
        <section id="deals"
          ...
          <div class="flex items-start justify-between">
            ...
            <!-- <router-link
              :to="`/sales_management/accounts/createDeal?parent_id=${
                route.params.id
              }&company_name=${
            ...
            <div v-else class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200 table-zebra">
                <thead class="bg-gradient-to-r from-emerald-50 to-teal-50">
                  <tr>
                    ...
                    <th v-for="field in dealFields" :key="field.id"
                      class="px-6 py-4 text-left text-nowrap text-xs border-e font-bold text-emerald-700 uppercase tracking-wider">
                      {{ field.label }}
                    </th>
                  </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-100">
                  ...
                  <tr v-else-if="dealsData.length == 0" class="hover:bg-emerald-50 transition-colors">
                    <td colspan="16" class="px-6 py-12 text-center text-gray-500">
                      <div class="flex flex-col items-center gap-4">
                        <div class="relative">
                          <div class="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center">
                        ...
                        <div class="text-center">
                          ...
                          <router-link
                            :to="`/sales_management/leads/createDeal?parent_id=${
                              route.params.id
                            }&company_name=${
                              singleLeads.find((a) => a.field?.name === 'company_name')?.value
                            }&parent_company=${
        ...
        <section id="contacts"
          ...
          <div class="flex items-start justify-between">
            ...
            <router-link :to="{
              name: 'sales_management-customer-assignContacts',
              query: {
                      ...
                      <div class="flex flex-col items-center gap-4">
                        ...
                        <div class="text-center">
                          ...
                          <router-link :to="{
                            ...
                            query: {
                              deal_id: route.params.id,
                              account_id: singleLeads.find((a) => a.field?.name === 'account_id')
                                ?.value,
                              company_name: singleLeads.find(
                                (a) => a.field?.name === 'company_name'
                              )?.value,
                              parent_company: singleLeads.find(
                                (a) => a.field?.name === 'parent_company_name'
                              )?.value
                            }
                          }"
                            class="bg-gradient-to-r from-emerald-500 to-teal-600 text-white px-6 py-3 rounded-xl hover:from-emerald-600 hover:to-teal-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center gap-2 mx-auto">
...
const fetchDealFields = async () => {
  const { data } = await api().get('/crm/modules/7/fields')
...
const convertToCustomer = async () => {
  ...
  try {
    Swal.fire({
      ...
      text: 'This account will be converted to a Customer.',
      icon: 'question',
...
const fetchContacts = async () => {
  try {
    ...
    const { data } = await api().get(
      `/crm/record-child-get/${route.params.id}/accounts-Contacts?field=account_contact_type&value=customer`
    )
...
const fetchDeals = async () => {
  try {
    isLoading.value = true
    const { data } = await api().get(`/crm/record-child-get/${route.params.id}/Accounts-Projects`)
