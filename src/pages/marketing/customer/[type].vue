...
const buildApiUrl = () => {
  const base = '/crm/modules/2/records'
  ...
  if (['today', 'thisWeek', 'nextWeek', 'lastWeek', 'thisMonth', 'nextMonth', 'lastMonth'].includes(type)) {
    params.append('date_filters[call_schedule_date][start]', startDate.value)
    params.append('date_filters[call_schedule_date][end]', endDate.value)
  }
    ...
    params.append('filters[lead_to_prospect_to_potential]', statusMap[type])
  }


  else if (type && ['bronze', 'silver', 'gold', 'diamond', 'platinum'].includes(type)) {
    params.append('filters[lead_priority]', type.charAt(0).toUpperCase() + type.slice(1))
  }

  params.append('filters[account_type_new]', 'customer')
        ...
        <div v-else class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 table-zebra">
            <thead class="bg-gradient-to-r from-emerald-50 to-teal-50">
              <tr>
                <th class="px-6 py-4 text-left border-x text-xs font-bold text-emerald-700 uppercase tracking-wider">
