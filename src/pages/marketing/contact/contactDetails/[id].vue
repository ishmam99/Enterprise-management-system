  ...
  <div class="flex h-screen bg-gray-50 pb-10 mt-2">
    <aside
      ...
      <button
        ...
        <span class="font-semibold text-sm">All Contacts</span>
      </button>
      ...
      <div class="bg-white shadow-sm px-8 py-5 border-b border-gray-200">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex gap-2 flex-row-reverse">
            ...
            <div
              v-if="lead?.values.find((e) => e.field?.name == 'account_type_new')?.value == '' || lead?.values.find((e) => e.field?.name == 'account_type_new')?.value == 'Not Customer'"
            ...
            <div v-if="lead?.values.find((e) => e.field?.name == 'account_type_new')?.value == 'Customer'"
              ...
              <button>Already A Customer</button>
            </div>
            <button
...
const saveEdit = async (item) => {
  ...
  try {
    await api().post(`/crm/record-values/${item.id}`, {
      value: editValue.value ? editValue.value : '',
      _method: 'PUT'
    })
...
const sidebarItems = [
  { key: 'overview', label: 'Contact Details' },
...
const fetchLead = async (page = 1) => {
  try {
    ...
    const { data } = await api().get(`/crm/modules/3/records/${route.params.id}`)
...
const convertToCustomer = async () => {
  ...
  if (!email) {
    Swal.fire({
      icon: "error",
      title: "Error!",
      text: "Email not fount. Customer must have an email.",
      timer: 1500,
  ...
  try {
    ...
    // 2. Prepare Payload for /customers API
    // Helper function to find values by field name


    // Construct Address: filter out empty parts and join with commas
    const addressParts = [
      getVal('street'),
      getVal('city'),
      getVal('state'),
...
const fetchDeals = async () => {
  try {
    ...
    // Filter out deals where status == "Completed"
    dealsData.value = data.data.filter(deal => {
      const status = deal.values.find(v => v.field?.name === "status")?.value
...
const fetchNotes = async () => {
  try {
    isLoading.value = true
    const { data } = await api().get(`/crm/record-child-get/${route.params.id}/Contacts-Notes`)
