<template>
  <div class=" px-4 py-6 w-full mx-auto ">
    ...
    <div class="bg-gradient-to-r from-teal-600 to-emerald-600 rounded-t-2xl text-white px-6 py-5 shadow-lg">
      <h1 class="text-3xl font-bold">🎯 Certification Request Lists</h1>
      <p class="text-teal-100 mt-1">Manage and organize your certification requests</p>
    </div>
    ...
    <div class="">
      ...
      <div class="my-8 bg-white rounded-2xl p-6 shadow-lg border border-teal-100">
        ...
        <div class="mt-6 flex gap-4 justify-end">
          ...
          <button
            @click="applyFilters"
            :disabled="!userForm.solutionId || !userForm.softwareId"
            class="px-6 py-3 bg-gradient-to-r from-teal-500 to-emerald-600 text-white rounded-xl font-semibold hover:from-teal-600 hover:to-emerald-700 transition-all duration-300 transform hover:scale-105 shadow-lg disabled:opacity-50 disabled:transform-none flex items-center"
          >
            🎯 Apply Filters
          </button>
        </div>
        ...
        <table class="min-w-full table-fixed border-collapse">
            ...
            <tbody>
              ...
              <td class="px-4 py-2 text-gray-700 text-sm break-words whitespace-normal">
                  <div class="font-semibold text-gray-800 flex items-center text-wrap">
                    {{ training.title }}
                  </div>
                </td>
...
const fetchTrainings = async (filters = {}) => {
  ...
  try {
    let apiUrl = `/training-course?per_page=15&status=2`
...
// ✅ FIXED: Apply filters with proper validation
const applyFilters = () => {
  ...
  if (!userForm.solutionId) {
    errors.solutionId = 'Please select a solution'
  }

  if (!userForm.softwareId) {
    errors.softwareId = 'Please select a software'
  }

  if (errors.solutionId || errors.softwareId) {
    return
  }
