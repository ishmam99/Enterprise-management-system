<template>
  <div class="w-full px-10 py-4 bg-slate-100 min-h-screen">
    <div class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-2xl p-6 mb-6 shadow-lg">
      <div class="flex items-center justify-between">
        ...
        <button
          ...
          @click="router.push({
            path: '/user/Career/BySoftware/create',
            query: { software_id: route.params.id, name: route.query.name }
          })"
        >
          + Add Development
        </button>
          ...
          <tbody>
            <tr v-if="loading">
             ...
             <tr v-else-if="!loading && developments.length == 0">
              <td colspan="50" class="text-center p-10 text-slate-400">
                No developments found for this software.
              </td>
            </tr>
            <tr
...
const fetchData = async (id) => {
  ...
  try {
    loading.value = true
    const { data } = await api().get(`/end-user-road-maps?software_id=${id}`)
...
watch(
  ...
  ([newId, newName]) => {
    softwareName.value = newName || 'Software'
    fetchData(newId)
  },
  { immediate: true }
)
