...
<div v-if="form.type == 'training'">
  ...
  No training found for this solution
</p>
</div>
...
// 👇 GET FROM QUERY
const softwareId = computed(() => String(route.query.solution_id || ''))
const softwareName = computed(() => route.query.name || 'Solution')
...
const getTrainingCourses = async () => {
  try {
    ...
    const { data } = await api().get(`/training-course?solution_id=10`)
