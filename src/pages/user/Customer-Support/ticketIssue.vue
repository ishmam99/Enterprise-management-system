      ...
      <div class="mb-8 p-6 bg-gradient-to-r from-indigo-100 to-purple-100 border-2 border-indigo-300 rounded-2xl shadow-lg">
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <!-- <p class="text-sm text-indigo-600 mt-1">User ID: {{ store.user?.profile?.id }}</p> -->
          </div>
...
// Computed properties for the required fields
const customerId = computed(() => store.user?.profile?.customer_id)
const userId = computed(() => store.user?.profile?.id)
...
const createTicket = async () => {
  ...
  // Check if we have the required customer and user data
  if (!customerId.value) {
    Swal.fire('Error', 'Customer information not found. Please try logging in again.', 'error')
    return
  }

  if (!userId.value) {
    Swal.fire('Error', 'User information not found. Please try logging in again.', 'error')
    return
  }
  ...
  try {
    ...
    // Updated to use the correct fields from store
    formData.append('customer_id', customerId.value)
    formData.append('user_id', userId.value)
    formData.append('end_user_id', userId.value) // Auto-selected: same as user_id
