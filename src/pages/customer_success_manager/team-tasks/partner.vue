...
const loadTasks = async () => {
  loading.value = true
  try {
    const res = await api().get(`/success-team-tasks?success_team_id=${authStore.team_id}&where=[{"column":"user_id","operator":"!=","value":"${authStore.user.id}"}]&with=solution,software,user,assignedPerson,outputs,successTeam`)
