...
const loadAssignments = async () => {
  loading.value = true
  try {
    const res = await api().get(`/success-team-tasks??success_team_id=${authStore.team_id}&software_id=${route.params.id}&with=solution,software,user,assignedPerson,outputs,successTeam`)
          ...
          <div class="flex-1">
            <div class="flex items-center gap-3 mb-3">
              <div class="w-3 h-8 rounded-full bg-gradient-to-b from-blue-500 to-purple-500"></div>
               <h1 class="text-3xl font-bold text-gray-900">Task List By Software  : {{ route.query.name }}</h1>
            </div>
