    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
      <div class="bg-gradient-to-r from-teal-700 to-teal-800 text-white px-6 py-4">
        <div class="flex justify-between items-center">
          <h1 class="text-2xl font-semibold">Usability Of Current Solutions</h1>
      ...
      <div class="p-6">
        ...
        <div class="mb-6 flex flex-col sm:flex-row gap-4 justify-between">
          <div class="relative flex-1 max-w-md">
            <input v-model="searchQuery" type="text" placeholder="Search by solution or customer..."
              ...
              <tr v-for="solution in paginatedSolutions" :key="solution.id" class="hover:bg-gray-50 transition">
                ...
                <td class="px-6 py-4 border-e border-slate-200">
                  <div class="flex items-center">
                    ...
                    <div class="ml-3">
                      ...
                      <input v-else v-model="solution.editName" type="text"
                        class="text-sm font-medium text-gray-900 border border-gray-300 rounded-lg px-3 py-1 w-full"
                        @keyup.enter="updateSolution(solution)" @keyup.escape="cancelEdit(solution)" />
                    </div>
                  </div>
                </td>
                ...
                <td class="px-6 py-4 border-e border-slate-200">
                  <div v-if="solution.customer" class="text-sm">
                    ...
                    <div class="text-xs text-gray-400">ID: {{ solution.customer_id }}</div>
                  </div>
                  <div v-else class="text-sm text-gray-400">No customer assigned</div>
                </td>
                ...
                <td class="px-6 py-4 border-e border-slate-200">
                  <div class="flex flex-col gap-1">
                    <div class="flex flex-wrap gap-1 max-w-xs">
                      <span v-for="software in solution.softwares" :key="software.id"
                    ...
                    <span v-if="solution.softwares?.length" class="text-xs text-gray-500 mt-1">
                      {{ solution.softwares.length }} software{{
                        solution.softwares.length !== 1 ? 's' : ''
                      }}
                    </span>
                  </div>
                </td>
                ...
                <td class="px-6 py-4 border-e border-slate-200 whitespace-nowrap">
                  <!-- VIEW MODE -->
                  <div v-if="!solution.isEditing">
                    <div class="w-full bg-gray-200 rounded-full h-2.5">
                      <div class="h-2.5 rounded-full transition-all duration-300"
                        :class="getProgressBarColor(solution.usability)"
                        :style="{ width: Number(solution.usability) + '%' }">
                      </div>
                    </div>

                    <div class="flex items-center justify-between mt-1">
                      <span class="text-xs text-gray-500">
                        {{ solution.usabilityValue }}%
                      </span>

                      <button @click="startEdit(solution)" class="text-xs text-blue-600 hover:underline">
                        Edit
                      </button>
                    </div>
                  </div>
                  ...
                  <div v-else class="space-y-2 min-w-[200px]">
                    ...
                    <div class="flex items-center gap-3">
                      ...
                      <input type="range" min="1" max="100" step="1" v-model.number="solution.tempUsability"
                    ...
                    <div class="flex items-center justify-between gap-2">
                      <div
                        class="text-sm font-semibold text-teal-700 bg-teal-50 px-2 py-1 rounded min-w-[60px] text-center">
                        {{ solution.tempUsability }}%
                      </div>

                      <div class="flex gap-1">
                        <button @click="saveUsability(solution)" :disabled="solution.isSaving"
                          ...
                          <svg v-if="solution.isSaving" class="animate-spin h-3 w-3" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="white" stroke-width="4"
                              fill="none" />
                            <path class="opacity-75" fill="white"
                              d="M4 12a8 8 0 018-8v4l3-3-3-3v4a10 10 0 00-10 10h4z" />
                          </svg>
                          {{ solution.isSaving ? 'Saving...' : 'Save' }}
                        </button>

                        <button @click="cancelEdit(solution)"
                ...
                <td class="px-6 py-4 border-e border-slate-200 whitespace-nowrap">
                  <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="solution.status === '0'
                ...
                <td class="px-6 py-4 border-e border-slate-200 whitespace-nowrap text-right text-sm font-medium">
                  ...
                  <!-- View Details -->
                  <button @click="viewDetails(solution)"
                    class="text-teal-600 flex items-center gap-1 hover:text-teal-900 mr-3" title="View details">
                    <svg class="h-5 w-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div>
                <p class="text-sm text-gray-700">
                  ...
                  <span class="font-medium">{{
                    Math.min(currentPage * itemsPerPage, filteredSolutions.length)
                    }}</span>
                  of
      ...
      <div class="relative mx-auto p-6 border w-full max-w-md shadow-lg rounded-lg bg-white" @click.stop>
        ...
        <form @submit.prevent="addSolution" class="space-y-4">
          <div>
            ...
            <input v-model="newSolution.name" type="text" required placeholder="Enter solution name"
          ...
          <div>
            ...
            <textarea v-model="newSolution.description" rows="3" placeholder="Enter solution description"
              class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none"></textarea>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1"> Status </label>
            <select v-model="newSolution.status"
              class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none">
              <option value="0">Active</option>
              <option value="1">Inactive</option>
            </select>
          ...
          <div class="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" :disabled="addLoading"
              ...
              <svg v-if="addLoading" class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg"
        ...
        <div v-if="selectedSolution" class="space-y-4">
          ...
          <div class="bg-gray-50 p-4 rounded-lg">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p class="text-sm">
                  <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="selectedSolution.status === '0'
                      ? 'bg-green-100 text-green-800'
                      : 'bg-gray-100 text-gray-800'
                    ">
            ...
            <div class="space-y-2">
              <div class="flex items-start">
                ...
                <div class="text-sm font-medium flex-1">
                  {{ selectedSolution.customer.user?.name }}
                </div>
              </div>
              <div class="flex items-start">
          ...
          <div v-if="selectedSolution.softwares?.length" class="bg-gray-50 p-4 rounded-lg">
            <h4 class="font-medium text-gray-700 mb-2">
              Software Products ({{ selectedSolution.softwares.length }})
            </h4>
            <div class="grid grid-cols-2 gap-2">
              <div v-for="software in selectedSolution.softwares" :key="software.id"
...
const filteredSolutions = computed(() => {
  return solutions.value.filter((solution) => {
    ...
    const matchesSearch =
      ...
      solution.softwares?.some((s) =>
        s.name.toLowerCase().includes(searchQuery.value.toLowerCase())
      )

    // Status filter
    const matchesStatus = statusFilter.value === 'all' || solution.status === statusFilter.value
...
const fetchSolutions = async () => {
  ...
  try {
    ...
    solutions.value = (response.data || []).map((solution) => ({
      ...
      // ✅ usability system (same as previous page)
      usabilityValue: Number(solution.usability || 0),
      tempUsability: Number(solution.usability || 0),
      isEditing: false,
      isSaving: false
    }))
  } catch (error) {
...
const startEdit = (solution) => {
  solution.isEditing = true
  solution.tempUsability = solution.usabilityValue
}

const cancelEdit = (solution) => {
  solution.isEditing = false
  solution.tempUsability = solution.usabilityValue
}

const saveUsability = async (solution) => {
  solution.isSaving = true

  try {
    await api().post(`customer-solution/${solution.id}`, {
      _method: 'PUT',
      usability: solution.tempUsability
    })

    solution.usabilityValue = solution.tempUsability
    solution.usability = solution.tempUsability
    solution.isEditing = false
