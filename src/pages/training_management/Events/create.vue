<template>
  <div class="px-2 bg-white py-5 w-3/4 mx-auto">
    <div class="bg-teal-700 rounded-t-2xl text-white px-6 py-4">
      ...
      <p class="text-gray-300">Fill out the form below to create a new eventdd</p>
    </div>
      ...
      <form @submit.prevent="createEvent">
        <div class="py-5 space-y-7 mt-5">
          ...
          <div class="flex items-center justify-center gap-5">
            ...
            <div class="relative mb-3 w-1/2">
              <label class="block text-sm font-semibold text-gray-700 mb-2">
                B2. Solution <span class="text-gray-500">(Auto Selected)</span>
              </label>
              <input
                type="text"
                class="peer block min-h-[auto] w-full rounded-xl border-2 px-3 py-[0.32rem] leading-[1.6] outline transition-all duration-200 ease-linear focus:placeholder:opacity-100 peer-focus:text-primary placeholder:opacity-100 motion-reduce:transition-none bg-gray-50"
          ...
          <!-- Auto-populated software field -->
            ...
            <div class="relative mb-3 w-1/2">
              <label class="block text-sm font-semibold text-gray-700 mb-2">
                B3. Software <span class="text-gray-500">(Auto Selected)</span>
              </label>
              <input
                type="text"
                class="peer block min-h-[auto] w-full rounded-xl border-2 px-3 py-[0.32rem] leading-[1.6] outline transition-all duration-200 ease-linear focus:placeholder:opacity-100 peer-focus:text-primary placeholder:opacity-100 motion-reduce:transition-none bg-gray-50"
          ...
          <div class="flex items-center justify-center gap-5">
            <div class="relative mb-3 w-1/2">
              <label class="block text-sm font-semibold text-gray-700 mb-2">
                B4. Training Type <span class="text-red-500">*</span>
              </label>
              <select
            ...
            <div class="relative mb-3 w-1/2">
              <label class="block text-sm font-semibold text-gray-700 mb-2">
                B5. Platform <span class="text-red-500">*</span>
              </label>
              <input
                type="text"
          ...
          <div class="flex items-center justify-center gap-5">
            <div class="relative mb-3 w-full">
              <label class="block text-sm font-semibold text-gray-700 mb-2">
                B6. Instructor <span class="text-red-500">*</span>
              </label>
              <select
...
const fetchTrainingCourses = async () => {
  ...
  try {
    const response = await api().get('training-course?with=software,solution,industry')
    const allCourses = response.data.data || response.data
    trainingCourses.value = allCourses.filter(course => course.status == 2)
  } catch (error) {
