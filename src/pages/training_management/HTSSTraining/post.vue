<template>
  <div class="px-2 bg-white py-5 w-11/12 mx-auto">
    <div class="bg-teal-700 rounded-t-2xl text-white px-6 py-4">
      ...
      <p class="text-gray-300">Review and post training requests</p>
    </div>
              ...
              <td class="border border-gray-300 px-4 py-3">
                <div class="flex gap-2">
                  ...
                  <button
                    ...
                    class="px-3 py-2 rounded-md shadow-md bg-red-600 text-white text-sm"
                    :disabled="deletingId === training.id"
                  >
                    <span v-if="deletingId === training.id" class="loading loading-spinner loading-xs"></span>
...
const statusUpdate = async (training) => {
  try {
    ...
    Swal.fire({
      title: 'Posted!',
      text: `Training has been posted successfully.`,
      icon: 'success'
    })
    ...
    Swal.fire({
      title: 'Error',
      text: 'Failed to post training. Please try again.',
      icon: 'error'
    })
  }
}
