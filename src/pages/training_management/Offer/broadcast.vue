<template>
  <div class="px-2 bg-white py-5 w-11/12 mx-auto">
    <div class="bg-teal-700 rounded-t-2xl text-white px-6 py-4">
      <h1 class="text-2xl font-semibold">Offer Broadcast</h1>
      <p class="text-gray-300">Review and broadcast offers</p>
    </div>
            ...
            >
              ...
              <td class="border border-gray-300 px-4 py-3">
                <div class="flex gap-2">
                  ...
                  <!-- Broadcast Button -->
                  <button
                    class="px-3 py-2 rounded-md shadow-md bg-yellow-600 text-white text-sm hover:bg-yellow-700 transition-colors"
                    @click="statusUpdate(offer)"
                    :disabled="actionLoading"
                  >
                    <span v-if="actionLoading && broadcastingId === offer.id" class="loading loading-spinner loading-xs"></span>
                    Broadcast
                  </button>
      ...
      <div class="modal-box max-w-4xl">
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
          ...
          <div>
            ...
            <p class="text-gray-900 capitalize">{{ selectedOffer.platform || 'N/A' }}</p>
          </div>
          <div>
...
const fetchOffers = async () => {
  ...
  try {
    ...
    offers.value = response.data.data.filter(e => e.status == 1) || response.data.filter(e => e.status == 1)
  } catch (error) {
...
const statusUpdate = async (offer) => {
  ...
  try {
    ...
    Swal.fire({
      title: 'Broadcasted!',
      text: `Offer has been Broadcasted.`,
      icon: 'success'
    })
    ...
    Swal.fire({
      title: 'Error',
      text: 'Failed to Broadcast. Please try again.',
      icon: 'error'
    })
  } finally {
    actionLoading.value = false
