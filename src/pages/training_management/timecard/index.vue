  ...
  <div class="w-11/12 mx-auto pt-6 px-10 space-y-4 min-h-screen">
    ...
    >
      ...
      <div class="flex justify-center items-center gap-4 my-4">
        <select v-model="selectedRange" class="rounded py-1" @change="onRangeChange">
          <option value="weekly">Weekly</option>
          <option value="monthly">Monthly</option>
        </select>

        <button class="px-4 py-1 bg-gray-200 rounded" @click="changeWeek(-1)">
          ◀ Previous Week
        </button>
        ...
        <button class="px-4 py-1 bg-gray-200 rounded" @click="changeWeek(1)">Next Week ▶</button>
      </div>
    </div>
    <!-- {{ timesByDate }} -->
    <!-- TABLE -->
