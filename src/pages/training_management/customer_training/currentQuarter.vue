<template>
  <div class="px-2 bg-white py-5 w-11/12 mx-auto">
    <div class="bg-teal-700 rounded-t-2xl text-white px-6 py-4">
      <h1 class="text-2xl font-semibold">All Offered Trainings</h1>
      <p class="text-gray-300">Review all offered trainings</p>
    </div>
    ...
    <div class="bg-white rounded-xl p-5 w-full shadow-md mt-5">
      ...
      <div class="flex justify-between items-center mb-6">
        <div class="flex gap-4">
          <!-- Current Quarter Filter Toggle -->
          <div class="flex items-center gap-2">
            <label class="flex items-center cursor-pointer">
              <input 
                type="checkbox" 
                v-model="filters.currentQuarterOnly" 
                class="checkbox checkbox-sm checkbox-teal-600"
              />
              <span class="ml-2 text-sm font-medium text-gray-700">
                Current Quarter 
                <span class="text-teal-600 font-semibold">({{ getCurrentQuarter() }})</span>
              </span>
            </label>
          </div>
      ...
      <div v-if="filters.currentQuarterOnly" class="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
        <div class="flex items-center text-blue-700">
          <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
          </svg>
          <span class="font-medium">Showing trainings for: {{ getCurrentQuarter() }}</span>
        </div>
      </div>
            ...
            >
             ...
             <!-- Industry -->
              <td class="border border-gray-300 px-4 py-3 text-gray-600">
                {{ offer.event.training_course.industry.name }}
              </td>
      ...
      <div v-if="filteredOffers.length > 0" class="flex justify-between items-center mt-6">
        <div class="text-sm text-gray-600">
          Showing {{ filteredOffers.length }} of {{ filteredOffers.length }} records
        </div>
        <div class="join">
...
// Check if a date is in current quarter
const isInCurrentQuarter = (dateString) => {
  if (!dateString) return false
  
  const date = new Date(dateString)
  const now = new Date()
  
  return getQuarterNumber(date) === getQuarterNumber(now) && 
...
const fetchOffers = async () => {
  ...
  try {
    ...
    offers.value = response.data.data.filter(e => e.status == 2) || response.data.filter(e => e.status == 2)
  } catch (error) {
...
const filteredOffers = computed(() => {
  ...
  if (filters.currentQuarterOnly) {
    filtered = filtered.filter(offer => 
      isInCurrentQuarter(offer.start_date) || isInCurrentQuarter(offer.end_date)
    )
  }
