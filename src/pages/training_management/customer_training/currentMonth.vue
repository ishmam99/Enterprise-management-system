    ...
    <div class="bg-white rounded-xl p-5 w-full shadow-md mt-5">
      ...
      <div class="flex justify-between items-center mb-6">
        <div class="flex gap-4">
          <!-- Current Month Filter Toggle -->
          <div class="flex items-center gap-2">
            <label class="flex items-center cursor-pointer">
              <input 
                type="checkbox" 
                v-model="filters.currentMonthOnly" 
                class="checkbox checkbox-sm checkbox-teal-600"
              />
              <span class="ml-2 text-sm font-medium text-gray-700">
                Current Month 
                <span class="text-teal-600 font-semibold">({{ getCurrentMonthYear() }})</span>
              </span>
            </label>
          </div>
      ...
      <!-- Current Month Info -->
      <div v-if="filters.currentMonthOnly" class="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
        <div class="flex items-center text-blue-700">
          <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
          </svg>
          <span class="font-medium">Showing trainings for: {{ getCurrentMonthYear() }}</span>
        </div>
      </div>
...
const filteredOffers = computed(() => {
  ...
  if (filters.currentMonthOnly) {
    filtered = filtered.filter(offer => 
      isInCurrentMonth(offer.start_date) || isInCurrentMonth(offer.end_date)
    )
  }
