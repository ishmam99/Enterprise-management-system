  ...
  <div class="p-6 w-4/5 bg-gray-50 min-h-screen mx-auto">
    ...
    <div v-if="selectedSolution" class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
      ...
      <div class="flex justify-between gap-3 items-center">
        <button class="btn btn-warning" @click="selectedSolution = null, getTrainingCourses(false)">
      ...
      <table class="min-w-full text-sm text-gray-700 table-zebra">
        ...
        <tbody>
          ...
          >
            ...
            <td class="py-3 border-e px-4">{{ course.solution?.name }}</td>
    ...
    <div v-else class="overflow-x-auto bg-white shadow-md rounded-xl border border-gray-100">
      <table class="min-w-full text-sm text-gray-700 table-zebra">
        ...
        <tbody>
          <tr
            v-for="(solution, index) in solutionStats"
            :key="solution.id"
            class="border-b border-gray-100 hover:bg-gray-50 transition"
          >
            <td class="py-3 border-e px-4">{{ index + 1 }}</td>
            <td class="py-3 border-e px-4 font-semibold">{{ solution.solution?.name }}</td>
            <td class="py-3 border-e px-4">{{ solution.total }}</td>
            <td class="py-3 border-e px-4">
              <button
                class="btn btn-info text-white"
                @click="selectedSolution = solution.solution_id, getTrainingCourses(true)"
              >
                View Courses
              </button>
            </td>
          ...
          <tr v-if="solutionStats.length === 0">
            <td colspan="4" class="text-center py-6 text-gray-400">No solutions found</td>
          </tr>
        </tbody>
      </table>
      <div v-if="solutionStats.length > 0 && meta" class="flex px-4 py-2 justify-between items-center mt-6">
        <div class="text-sm text-gray-600">
          Showing {{ solutionStats.length }} of {{ meta.total }} records from {{ meta.from }} to {{ meta.to }}
        </div>
        <div class="bg-cyan-500 space-x-1 px-2 rounded-md py-1">
          <button
            v-for="link in meta.links"
    ...
    <div v-if="selectedCourse" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl shadow-2xl w-[90%] sm:w-[500px] p-6 relative">
        ...
        <h2 class="text-xl font-bold text-emerald-700 mb-3">{{ selectedCourse.title }}</h2>
        <p><strong>Training Level:</strong> {{ selectedCourse.training_level }}</p>
        <p><strong>Duration:</strong> {{ selectedCourse.duration }}</p>
        <p><strong>Solution:</strong> {{ selectedCourse.solution?.name }}</p>
        <p><strong>Status:</strong> {{ getStatusText(selectedCourse.status) }}</p>
        <p class="mt-3 text-gray-700">{{ selectedCourse.description || 'No description available' }}</p>
      </div>
    </div>
...
const getTrainingCourses = async (showCourses) => {
  try {
    ...
    if (showCourses) {
      // Show training courses for selected solution
      let whereClause = `[{"column":"solution_id","operator":"=","value":"${selectedSolution.value}"}]`
      
      queryParams = `?per_page=20&page=${currentPage.value}&where=${whereClause}&with=solution&search=${searchQuery.value}`
      
      const { data } = await api().get(`training-course${queryParams}`)
      trainingCourses.value = data.data.data
      meta.value = data.meta
    } else {
      // Show solution statistics (grouped by solution)
      queryParams = `?group_by=solution_id&with=solution`
      
      const { data } = await api().get(`training-course${queryParams}`)
      solutionStats.value = data.data
    }
  } catch (error) {
