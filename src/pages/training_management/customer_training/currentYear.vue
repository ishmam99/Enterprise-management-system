    ...
    <div class="bg-white rounded-xl p-5 w-full shadow-md mt-5">
      ...
      <div class="flex justify-between items-center mb-6">
        <div class="flex gap-4">
          <!-- Current Year Filter Toggle -->
          <div class="flex items-center gap-2">
            <label class="flex items-center cursor-pointer">
              <input 
                type="checkbox" 
                v-model="filters.currentYearOnly" 
                class="checkbox checkbox-sm checkbox-teal-600"
              />
              <span class="ml-2 text-sm font-medium text-gray-700">
                Current Year 
                <span class="text-teal-600 font-semibold">({{ getCurrentYear() }})</span>
              </span>
            </label>
          </div>
      ...
      <!-- Current Year Info -->
      <div v-if="filters.currentYearOnly" class="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
        <div class="flex items-center text-blue-700">
          <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
          </svg>
          <span class="font-medium">Showing trainings for: {{ getCurrentYear() }}</span>
        </div>
      </div>
...
// Check if a date is in current year
const isInCurrentYear = (dateString) => {
...
const filteredOffers = computed(() => {
  ...
  if (filters.currentYearOnly) {
    filtered = filtered.filter(offer => 
      isInCurrentYear(offer.start_date) || isInCurrentYear(offer.end_date)
    )
  }
