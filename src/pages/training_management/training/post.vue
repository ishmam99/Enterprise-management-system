<template>
  <div class="min-h-screen w-full bg-gray-50">
    <!-- Header Section -->
    <div class="bg-green-200 shadow-sm border-b border-gray-200 sticky top-0 z-30">
      <div class="container mx-auto px-4 py-4">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 class="text-2xl md:text-3xl font-bold text-green-900">Approved for Publish Trainings List</h1>
            <p class="text-gray-500 text-sm mt-1">Manage and track all training programs</p>
          </div>
          <RouterLink 
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right">
                    <div class="flex items-center justify-end gap-2">
                      ...
                      <button 
                        ...
                        class="p-1.5 hover:text-blue-100 hover:bg-blue-600 rounded-lg transition bg-blue-500 text-white"
                        :title="training.status == 2 ? 'Deactivate' : 'Publish'"
                      >
                      Publish
                      </button>
                      <button 
...
const fetchTrainings = async () => {
  try {
    const response = await api().get(`/internal-trainings?status=1`);
    let trainingsData = [];
