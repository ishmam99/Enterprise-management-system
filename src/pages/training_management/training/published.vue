<template>
  <div class="min-h-screen w-full bg-gray-50">
    <!-- Header Section -->
    <div class="bg-blue-500  shadow-sm border-b border-gray-200 sticky top-0 z-30">
      <div class="container mx-auto px-4 py-4">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 class="text-2xl md:text-3xl font-bold text-white text-center">Published on Website Trainings List</h1>
            <p class="text-gray-100 text-sm mt-1">Manage and track all training programs</p>
          </div>
          <RouterLink 
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right">
                    <div class="flex items-center justify-end gap-2">
                      ...
                      <button 
                        ...
                        :title="training.status == 1 ? 'Deactivate' : 'Activate'"
                      >
...
const fetchTrainings = async () => {
  try {
    const response = await api().get(`/internal-trainings?status=2`);
    let trainingsData = [];
