      ...
      <main class="px-8 pb-10 flex-1 space-y-6">
        ...
        >
          <div class="flex items-start justify-between">
            ...
            <router-link
              :to="{
                name: 'sales_management-deals-assignContacts',
                query: {
                  deal_id: route.params.id,
                  account_id: singleLeads.find((a) => a.field?.name === 'account_id')?.value,
...
const bottomGroup17 = computed(() => {
  return lead.value?.values?.filter((item) => Number(item.field.order_group) == 17) || []
})
...
const bottomGroup26 = computed(() => {
  return lead.value?.values?.filter((item) => Number(item.field.order_group) == 26) || []
})
const bottomGroup27 = computed(() => {
  return lead.value?.values?.filter((item) => Number(item.field.order_group) == 27) || []
})
const bottomGroup28 = computed(() => {
  return lead.value?.values?.filter((item) => Number(item.field.order_group) == 28) || []
})
const bottomGroup29 = computed(() => {
  return lead.value?.values?.filter((item) => Number(item.field.order_group) == 29) || []
})
...
const emptyBottomGroup17 = computed(() => {
  return emptyFields.value.filter((f) => Number(f.order_group) == 17)
})
...
const emptyBottomGroup26 = computed(() => {
  return emptyFields.value.filter((f) => Number(f.order_group) == 26)
})
const emptyBottomGroup27 = computed(() => {
  return emptyFields.value.filter((f) => Number(f.order_group) == 27)
})
const emptyBottomGroup28 = computed(() => {
  return emptyFields.value.filter((f) => Number(f.order_group) == 28)
})
const emptyBottomGroup29 = computed(() => {
  return emptyFields.value.filter((f) => Number(f.order_group) == 29)
})
...
const sections = computed(() => [
  ...
  {
    id: 'rapid-21',
    title: 'D. Sales Review Status: 2_Engg Service : Component',
  ...
  },
  {
    ...
    items: bottomGroup26.value || [],
    emptyItems: emptyBottomGroup26.value || [],
  },
  {
    id: 'rapid-27',
    title: 'F. Sales Action: 3_Senior Sales Activity',
    items: bottomGroup27.value || [],
    emptyItems: emptyBottomGroup27.value || [],
  },
  {
    id: 'rapid-28',
    title: 'F. Sales Action: 4_VIP Sales Activity',
    items: bottomGroup28.value || [],
    emptyItems: emptyBottomGroup28.value || [],
  },
  {
    id: 'rapid-29',
    title: 'G. Competitor Analysis',
    items: bottomGroup29.value || [],
    emptyItems: emptyBottomGroup29.value || [],
  },
])
...
const deleteRecord = async () => {
  ...
  try {
    ...
    router.push('/sales_management/deals/allDeals')
    await fetchLeads()
  } catch (err) {
    console.error(err)
    Swal.fire({
