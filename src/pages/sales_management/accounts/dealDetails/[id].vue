  ...
  <div class="flex h-screen bg-gray-50 pb-10 mt-2">
    <aside
      ...
      <button
        ...
        <span class="font-semibold text-sm">Back to Accounts</span>
      </button>
    ...
    <div class="flex-1 flex flex-col overflow-y-auto overflow-x-hidden">
        ...
        <div class="flex items-center justify-between">
          <div class="text-xl font-semibold">
            ...
            <p class="text-gray-600 mt-1 flex items-center">
              ...
              Deal Name:
              {{lead?.values.find((e) => e.field?.name == 'deal_name')?.value || 'N/A'}}
            </p>
          </div>
          <div class="flex gap-2 flex-row-reverse">
            <router-link :to="{
              name: 'sales_management-deals-create-Proposal',
              query: {
            ...
            <div
              ...
              <button @click="convertToProject()">Convert to Project</button>
            </div>
            <!-- <div
             v-if="lead?.values.find((e) => e.field?.name == 'account_type_new')?.value == 'Customer'"
      ...
      <div :id="'overview'" class="bg-blue-100 shadow-sm rounded-lg p-6 mx-8 my-6 border border-gray-200">
        <div class="flex items-center border-b mb-6 bg-gray-50 px-4 pt-4 rounded-t-lg">
          <button v-for="tab in tabs" :key="tab"
            class="px-6 py-3 text-sm font-medium flex gap-2 items-center rounded-t-lg transition-all mx-1" :class="overviewTab === tab
        ...
        <div v-if="overviewTab === 'Overview'" class="space-y-4 text-sm">
          <div class="bg-white rounded-b-xl shadow-md border border-gray-200 p-6">
            ...
            <div v-else class="grid md:grid-cols-5 gap-6">
              <div v-for="item in topGroup" :key="item.id"
              ...
              <div v-for="field in emptyTopGroup" :key="field.id"
                ...
                <template v-if="editingEmptyFieldId !== field.id">
                  <div class="flex justify-between items-start">
                    ...
                    <button @click="startEditEmpty(field)" class="text-blue-600 hover:text-blue-800 transition"
                      title="Add Value">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"
                        fill="currentColor">
      ...
      <div :id="'rapid'" class="bg-emerald-100 shadow-sm rounded-lg p-6 mx-8 my-6 border border-gray-200">
        ...
        <div class="space-y-4 text-sm">
          <div class="bg-white rounded-b-xl shadow-md border border-gray-200 p-6">
            ...
            <div v-else class="grid md:grid-cols-5 gap-6">
              <div v-for="item in bottomGroup" :key="item.id"
              ...
              <div v-for="field in emptyBottomGroup" :key="field.id"
                class="flex flex-col bg-white/40 border border-slate-200 rounded-xl px-5 py-3 shadow-sm hover:shadow-md transition-all">
                <template v-if="editingEmptyFieldId !== field.id">
                  <div class="flex justify-between items-start">
                    <div>
            ...
            <div v-else class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200 table-zebra">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                  ...
                  <tr v-else-if="dealContactsData.length == 0" class="hover:bg-emerald-50 transition-colors">
                  ...
                  <tr v-else v-for="(lead, index) in dealContactsData" :key="lead.id"
              ...
              <table class="min-w-full divide-y divide-gray-200 table-zebra">
                <thead class="bg-gradient-to-r from-emerald-50 to-teal-50">
                  <tr>
                    ...
                    <th v-for="field in moduleData2" :key="field.id"
                      class="px-6 py-4 text-left text-xs border-e font-bold text-emerald-700 uppercase tracking-wider">
                      {{ field.name }}
                    </th>
                  </tr>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                  ...
                  <tr v-else-if="notesData?.length == 0" class="hover:bg-emerald-50 transition-colors">
                    <td colspan="16" class="px-6 py-12 text-center text-gray-500">
                      <div class="flex flex-col items-center gap-4">
                        <div class="relative">
                          <div class="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center">
...
const fetchLead = async (page = 1) => {
  try {
    ...
    const { data } = await api().get(`/crm/modules/5/records/${route.params.id}`)
...
const topGroup = computed(() => {
  return lead.value?.values?.filter(
    item => Number(item.field.order_group) == 0 && item.field.name != 'account_type_new'
  ) || []
})
...
const emptyTopGroup = computed(() => {
  return emptyFields.value.filter(f => Number(f.order_group) === 0 && f.name != 'account_type_new')
})
...
const convertToProject = async () => {
  const field = fields.value.find(f => f.name === "status")
  ...
  const existing = lead.value.values.find(v => v.field?.name === "status")

  try {
    Swal.fire({
      title: "Convert Deal to Project?",
      text: "This Deal will be converted to a Project.",
      icon: "question",
      ...
      if (existing) {
        // UPDATE value
        const { data } = await api().get(`/crm/convert-deal-to-project/${route.params.id}`)
        ...
        // CREATE NEW value
        const res = await api().get(`/crm/convert-deal-to-project/${route.params.id}`)
        const { data } = await api().post(`/crm/create-values/${route.params.id}`, {
          field_id: field.id,
          value: "Completed"
        })

        lead.value.values.push({
          ...data.data,
          field: field
        })
      }
...
const removeFromCustomer = async () => {
  ...
  try {
    Swal.fire({
      ...
      text: "This account will be removed from Customer.",
      icon: "warning",
      showCancelButton: true,
      ...
      Swal.fire({
        icon: "success",
        title: "Updated!",
        text: "Account is no longer a customer.",
        timer: 1500,
        showConfirmButton: false
      })
    })
...
onMounted(async () => {
  ...
  dealStore.$patch({
    ...
    deal_name: lead?.value?.values.find((e) => e.field?.name == 'deal_name')?.value || '',
    software_name: lead?.value?.values.find((e) => e.field?.name == 'software_name')?.value || '',
    industry: lead?.value?.values.find((e) => e.field?.name == 'industry_name')?.value || '',
    service_type: lead?.value?.values.find((e) => e.field?.name == 'service_type')?.value || '',
    software_area: lead?.value?.values.find((e) => e.field?.name == 'service_area_name')?.value || '',
  })
  if (route.hash) {
