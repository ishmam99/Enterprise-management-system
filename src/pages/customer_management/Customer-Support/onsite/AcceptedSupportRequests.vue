...
<div class="mb-8">
  <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 p-6 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-2xl shadow-xl text-white">
    <div>
      <h1 class="text-3xl font-bold tracking-tight">Accepted Support Requests</h1>
...
const refreshTickets = async () => {
  try {
  //  const {data} = await api().get('customer-support?status=1&type=onsite')
      const {data} = authStore.role !== 'customer-manager' ? await api().get(`customer-support?status=1&type=onsite`) : await api().get(`customer-support?company_id=${authStore.companyId}&status=1&type=onsite`)
