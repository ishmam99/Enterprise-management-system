    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="mb-8 flex items-center justify-between">
        ...
        <div class="flex items-center space-x-4">
          <button @click="$router.push('/create-ticket')"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 sm:space-x-6">
          ...
          <div class="flex flex-wrap gap-4">
            <select v-model="filters.priority"
              ...
              <option value="Medium">Medium829</option>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-100 bg-white">
            <tr v-for="ticket in filteredTickets" :key="ticket.id"
              ...
              <td class="px-6 py-4">
                <span :class="getStatusClass(ticket.status)" class="px-3 py-1.5 rounded-full text-xs font-bold">
              ...
              <td class="px-6 py-4">
                <div v-if="ticket.attachment" class="flex justify-center">
                  <img :src="ticket.attachment" alt="Attachment"
                    class="w-16 h-16 object-cover rounded-xl border-2 border-purple-200 hover:border-purple-400 transition-all duration-300 cursor-pointer"
              ...
              <td class="px-6 py-4 text-center space-y-2">
                <button @click="viewTicket(ticket)"
                  class="block w-full px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white text-sm font-medium rounded-xl shadow-md hover:shadow-lg transition-all duration-300">
                  View Details
                </button>
                ...
                <div class="flex flex-col gap-2 mt-2">
                  <!-- In Progress -->
                  <button v-if="ticket.status === 1"
                    @click="updateTicketStatus(ticket.id, 2)"
                    :disabled="updatingId === ticket.id"
                    class="px-4 py-2 bg-gradient-to-r from-yellow-500 to-orange-600 hover:from-yellow-600 hover:to-orange-700 text-white text-xs font-medium rounded-xl shadow-md hover:shadow-lg transition-all disabled:opacity-60">
                    <span v-if="updatingId !== ticket.id">In Progress</span>
                  ...
                  <button
                    ...
                    class="px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white text-xs font-medium rounded-xl shadow-md hover:shadow-lg transition-all disabled:opacity-60"
                      @click="updateTicketStatus(ticket.id, 2)">
                    <span v-if="updatingId !== ticket.id">Approve</span>
                    <span v-else>Updating...</span>
                  </button>
                  ...
                  <!-- Final States -->
                  <span v-if="ticket.status === '2'" class="text-green-600 font-bold text-sm">Approved</span>
                  <span v-if="ticket.status === 4" class="text-red-600 font-bold text-sm">Rejected</span>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div v-else class="text-center py-20">
        <div class="inline-flex flex-col items-center max-w-md">
          ...
          <h3 class="text-2xl font-bold text-gray-900">No unresolved tickets</h3>
...
const updatingId = ref(null) // Track which ticket is being updated

const fetchTickets = async () => {
  isLoading.value = true
  try {
    // const response = await api().get(`/customer-support?type=online&status=0`)
    const response = authStore.role !== 'customer-manager' ? await api().get(`/customer-support?type=online&status=0`) : await api().get(`/customer-support?company_id=${authStore.companyId}&type=online&status=0`)
