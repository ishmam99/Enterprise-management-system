        ...
        <div>
          <div class="flex items-center mb-2">
            ...
            <h1 class="text-4xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
              Completed Live Chat Schedule
            </h1>
          </div>
          <p class="text-gray-600 ml-13">View and manage all scheduled live chats for completed sessions</p>
        </div>
...
const fetchTodayChats = async () => {
  ...
  try {
    ...
    const {data} = authStore.role !== 'customer-manager' ? await api().get('/customer-support?type=liveChat&today=1') : await api().get(`/customer-support?type=liveChat&today=1&customer_id=${authStore.companyId}`)
