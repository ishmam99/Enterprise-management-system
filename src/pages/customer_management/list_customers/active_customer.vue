<template>
  <div class="p-6 w-4/5 bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 min-h-screen mx-auto">
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-8">
      ...
      <select v-model="selectedIndustry" @change="getCustomers()"
    ...
    <div class="flex gap-4 mb-8">
      <button v-if="authStore?.user?.role === 'customer-director'" @click="openMassAssignModal('manager')"
        :disabled="selectedIds.length === 0" class="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl shadow-md
         hover:shadow-lg transform hover:scale-105 transition-all duration-200
         disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100">
        Mass Assign Manager
      </button>

      <button v-if="authStore?.user?.role === 'customer-director'" @click="openMassAssignModal('executive')"
        class="px-6 py-3 bg-gradient-to-r from-green-500 to-teal-600 text-white rounded-xl shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-200"
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-purple-100">
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gradient-to-r from-purple-600 to-pink-600 text-white">
            <tr>
              <th v-if="authStore?.user?.role === 'customer-director'" class="py-4 px-6 text-left">
              ...
              <th class="py-4 px-6 text-left">Manager</th>
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="(customer, index) in customers" :key="customer.id"
              ...
              <td v-if="authStore?.user?.role === 'customer-director'" class="py-1 px-6">
              ...
              <td class="py-1 px-6 text-gray-600 font-bold">{{ getManagerName(customer) || 'Unassigned' }}</td>
              <td class="py-1 px-6 text-gray-600 font-bold">{{ getExecutiveName(customer) || 'Unassigned' }}</td>
              <td class="py-1 px-6">
                <span :class="[
              ...
              <td class="py-1 px-6 text-center">
                <div class="flex justify-center gap-3">
                  ...
                  <button v-if="authStore?.user?.role === 'customer-director'" @click="approveRequest(customer)"
                  ...
                  <!-- View Details -->
                  <router-link :to="{ name: 'customer_management-CustomerDetails-id', params: { id: customer.id } }"
            ...
            <tr v-if="customers.length === 0">
              <td colspan="10" class="text-center py-16">
                <div class="flex flex-col items-center justify-center text-gray-400 text-lg">
                  ...
                  <span>No active customer found</span>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <!-- Pagination -->
      <div v-if="meta && customers.length > 0"
        ...
        <div class="flex gap-2">
          <button v-for="link in meta.links" :key="link.label"
            @click="currentPage = link.page || currentPage; getCustomers()" :class="[
    ...
    <div v-if="showMassAssignModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
        ...
        <select v-model="assignUserId"
          ...
          <option v-for="user in selectedAssignRole === 'manager' ? managers : executives" :key="user.id"
            :value="user.id">
            {{ user.name }}
          </option>
        </select>
        <div class="flex justify-end gap-4">
          ...
          <button @click="submitMassAssign" :disabled="assignLoading"
  ...
  try {
    const url =
      authStore.role == "customer-manager"
        ? `customers?status=1&with=user,assignedUsers.user,industry${industryFilter}&per_page=20&page=${currentPage.value}&search=${encodeURIComponent(searchQuery.value)}&company_id=${authStore.companyId}`
...
const getManagerName = (customer) => {
  const manager = customer.assigned_users?.find(a => a.role === 'customer-manager')
  return manager?.user?.name || 'Unassigned'
}
const getExecutiveName = (customer) => {
  const manager = customer.assigned_users?.find(a => a.role === 'customer-executive')
  return manager?.user?.name || 'Unassigned'
}
...
const fetchManagers = async () => {
  try {
    const { data } = await api().get(
      `/users?where=[{"column":"role","operator":"=","value":"customer-manager"}]`
    )
