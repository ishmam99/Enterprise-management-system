...
const getUsers = async (type) => {
  ...
  const url =
      authStore.role == "customer-manager"
        ? `training-course?with=software,solution,industry&company_id=${authStore.companyId}`
        : `training-course?with=software,solution,industry`
