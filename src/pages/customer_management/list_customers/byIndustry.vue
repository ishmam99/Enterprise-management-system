    ...
    <!-- Requests Table -->
      ...
      <table class="min-w-full text-sm text-gray-700 table-zebra">
        ...
        <tbody>
            ...
            <td colspan="6" class="text-center py-1 text-gray-400">No customers found</td>
          </tr>
          <tr v-for="(req, index) in customers" :key="req.id"
            ...
            <td class="py-1 border-e px-4">{{ req.phone }}</td>
            <td class="py-1 border-e px-4">{{ req.industry?.name }}</td>
            ...
            <td class="py-1 px-4 text-center flex justify-center gap-3">
              ...
              <!-- <button class="px-3 py-1 text-white btn btn-info rounded-lg hover:bg-blue-100 transition" @click="viewDetails(req)">
                View
                  ...
                  <!-- View Details -->
                  <router-link
                    :to="'/customer_management/CustomerDetails/'+ req.id"
    ...
    <div v-else class="overflow-x-auto bg-white shadow-md rounded-xl border border-gray-100">
      ...
      <table class="min-w-full text-sm text-gray-700 table-zebra">
        <thead class="bg-emerald-600 text-white text-left">
          <tr>
            <th class="py-3 border-e px-4">#</th>
            <th class="py-3 border-e px-4">Industry Name</th>
        ...
        <tbody>
          <tr v-for="(req, index) in customers" :key="req.id"
            class="border-b border-gray-100 hover:bg-gray-50 transition">
            <td class="py-1 border-e px-4">{{ index + 1 }}</td>
            <td class="py-1 border-e px-4 font-semibold">{{ req.industry?.name }}</td>
            <td class="py-1 border-e px-4">{{ req.total}}</td>
            <td class="py-1 border-e px-4"><button class="btn btn-info text-white" @click="selectedIndustry = req.industry_id,getCustomers(true)">View List</button></td>
...
const getCustomers = async (type) => {
  ...
  const url =
      authStore.role == "customer-manager"
        ? `customers?with=industry,user${groupBy}&status=1&${indStr}&company_id=${authStore.companyId}`
        : `customers?with=industry,user${groupBy}&status=1&${indStr}`
