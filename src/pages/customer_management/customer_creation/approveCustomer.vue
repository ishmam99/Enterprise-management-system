        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            >
              ...
              <td class="py-5 px-6 text-gray-600">{{ customer.user?.email }}</td>
              <td class="py-5 px-6 text-gray-600">{{ customer.phone }}</td>
              <td class="py-5 px-6">
              ...
              <td class="py-5 px-6 text-center">
                <div class="flex justify-center gap-3">
                  ...
                  <button
                    ...
                    class="px-5 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-200"
                  >
...
const getCustomers = async () => {
  ...
  if (selectedIndustry.value) {
    industryFilter = `,{"column":"industry_id","operator":"=","value":${selectedIndustry.value}}`
  }

  try {
    const { data } = await api().get(
      `customers?with=user,industry&where=[{"column":"status","operator":"=","value":"0"${industryFilter}}]&per_page=20&page=${currentPage.value}&search=${encodeURIComponent(searchQuery.value)}`
    )
...
getCustomers()
    // Show toast
    toastMessage.value = `${customer.user.name} has been approved!`
