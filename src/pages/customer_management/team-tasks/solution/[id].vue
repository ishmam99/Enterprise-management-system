...
const loadAssignments = async () => {
  ...
  try {
    const res = await api().get(
      `/success-team-tasks?success_team_id=${authStore.team_id}&solution_id=${route.params.id}&with=solution,software,user,assignedPerson,outputs,successTeam`
    )
...
const filteredAssignments = computed(() => {
  ...
  if (statusFilter.value !== 'all') {
    filtered = filtered.filter(
      (task) => task.status?.toLowerCase() === statusFilter.value.toLowerCase()
    )
  }

  if (assigneeFilter.value !== 'all') {
    filtered = filtered.filter(
      (task) => task.assigned_person?.id.toString() === assigneeFilter.value
    )
  }
...
<template>
  <div
    class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 pb-16 pt-10 px-4 sm:px-6 lg:px-8"
  >
    <div class="mx-auto max-w-7xl">
      <div class="mb-10">
          ...
          <div class="flex-1">
            <div class="flex items-center gap-3 mb-3">
              ...
              <h1 class="text-3xl font-bold text-gray-900">
                Task List By Solution : {{ route.query.name }}
              </h1>
            </div>
            <p class="text-gray-600 max-w-2xl">
              Track and manage success team tasks and assignments with real-time updates and
              progress monitoring.
            </p>
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          >
            ...
            <div class="mt-4 text-xs text-gray-500">Currently active tasks</div>
          </div>
          ...
          >
            ...
            <div class="mt-4 text-xs text-gray-500">Awaiting assignment or start</div>
          </div>
        ...
        <div class="flex flex-col lg:flex-row gap-6">
          ...
          <div class="flex items-center gap-3">
            ...
            <button
              ...
              :class="
                showFilters ? 'bg-blue-50 text-blue-600 border-blue-300' : 'bg-white text-gray-700'
              "
            ...
            >
              ...
              Filters
              <span
                v-if="statusFilter !== 'all' || assigneeFilter !== 'all'"
      ...
      <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <p class="text-sm text-gray-600">
          Showing
          <span class="font-semibold text-gray-900">{{ filteredAssignments.length }}</span> of
      ...
      <div v-if="loading" class="flex justify-center py-20">
        <div class="text-center">
          <div
            class="inline-block h-14 w-14 animate-spin rounded-full border-4 border-solid border-blue-500 border-r-transparent"
          ></div>
      ...
      <div
        v-else-if="viewMode === 'grid' && filteredAssignments.length > 0"
        ...
        >
          <div class="px-6 pt-6">
            <div class="flex items-center justify-between mb-4">
              ...
              <div class="relative group">
                ...
                <div
                  class="absolute right-0 top-full mt-2 w-48 rounded-xl bg-white shadow-lg border border-gray-100 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10"
                >
                  <button
                    class="w-full px-4 py-2.5 text-left hover:bg-gray-50 transition-colors flex items-center gap-2 text-gray-700"
                    @click="router.push(`/customer_management/team-tasks/${task.id}`)"
                  >
          ...
          <div class="px-6 py-4 bg-gray-50 border-t border-gray-100">
            <div class="space-y-3">
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 rounded-lg bg-gradient-to-br from-emerald-100 to-emerald-50 flex items-center justify-center"
                >
                  <i class="fas fa-desktop text-emerald-500"></i>
                </div>
                <div>
                  <div class="text-sm font-medium text-gray-900">
                    {{ task.software?.name || 'No software' }}
                  </div>
              ...
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-100 to-blue-50 flex items-center justify-center"
                >
                  <i class="fas fa-user text-blue-500"></i>
                </div>
                <div>
                  <div class="text-sm font-medium text-gray-900">
                    {{ task.assigned_person?.name || 'Unassigned' }}
                  </div>
                  <div class="text-xs text-gray-500">
                    {{ formatRole(task.assigned_person?.role) || 'No role' }}
                  </div>
                </div>
              </div>
              ...
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 rounded-lg bg-gradient-to-br from-amber-100 to-amber-50 flex items-center justify-center"
                >
      ...
      <div
        v-else-if="viewMode === 'table' && filteredAssignments.length > 0"
      ...
      <div v-else-if="!loading && filteredAssignments.length === 0" class="text-center py-16">
        <div class="max-w-md mx-auto">
          <div
            class="w-24 h-24 mx-auto mb-6 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center"
          >
