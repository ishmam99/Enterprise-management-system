<template>
  <div class="p-6 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 min-h-screen">
    <!-- Header Section -->
    <div class="mb-8 text-center">
      <h1 class="text-4xl font-bold mb-2 bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
List of User by Solutions     </h1>
      <p class="text-gray-600 max-w-2xl mx-auto">
        Browse and manage users by solution with this interactive dashboard
      </p>
    </div>

    <!-- solution Selection Card -->
    <div class="bg-white rounded-2xl shadow-lg p-6 mb-8 max-w-7xl mx-auto border border-indigo-100">
      <div class="flex items-center mb-4">
        ...
        <h2 class="text-xl font-semibold text-gray-800">Filter by solution</h2>
      </div>

      <label class="block text-lg font-medium text-gray-700 mb-3">Select solution</label>
      <div class="relative">
        <select
    ...
    <div class="max-w-full mx-auto">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">solution(s)</th>
              </tr>
            </thead>
      ...
      <div v-else class="text-center py-12">
        <div class="max-w-md mx-auto">
          ...
          <p class="text-gray-500 mb-6">Select a different solution or check back later for updates.</p>
          <button
...
const fetchsolutions = async () => {
  try {
    const response = await api().get('/customer-solution')
    // 👇 FIX: Directly assign response.data if API returns array
    solutions.value = response.data || []
    console.log('solutions loaded:', solutions.value)
  } catch (error) {
...
{
 return endUsers.value.filter(e=>e.solutions.find(c=>c.id == selectedsolution.value))
}
else return endUsers.value
})

// ✅ Fetch end users for selected solution
const fetchEndUsers = async () => {
  // if (!selectedsolution.value) return
  loading.value = true
  try {
    // Make sure the query parameter is correct for your backend
    const response = await api().get(`/end-users?with=user,solutions,solutions&customer_id=${store.user.customer.id}"}]`)
