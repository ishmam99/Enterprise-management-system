...
List of User by software     </h1>
    ...
    <div class="max-w-full mx-auto">
      ...
      <div v-else-if="endUsers.length" class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
        <div class="px-4 py-4 bg-gradient-to-r from-indigo-600 to-purple-600">
...
{
 return endUsers.value.filter(e=>e.softwares.find(c=>c.id == selectedsoftware.value))
}
...
const fetchEndUsers = async () => {
  ...
  try {
    ...
    const response = await api().get(`/end-users?with=user,softwares,solutions&where=[{"column":"customer_id","operator":"=","value":"${store.user.customer.id}"}]`)
